...
#wide to long
dissim_long <- melt(distances_dissimilarities_allyears.r, id.vars = c("year"), variable.name = "Dissimilarity metric")
...
distances_dissimilarities_allyears <- distances_dissimilarities_allyears[color_link, on = "survey_unit"]
...
points_wavylines_bray_total_year_reduced_gam_nolmer <- ggplot() +
 # geom_ribbon(data = lmer_bray_total_predictions, aes(x = year, ymin = bray_curtis_lmer_preds_lowerCI, ymax = bray_curtis_lmer_preds_upperCI), fill = "grey", alpha = 0.2) +
  geom_point(data = na.omit(distances_dissimilarities_allyears,cols = "year_adj"),
             aes(x = year,
...
points_wavylines_bray_total_year_reduced_gam_greyscale <- ggplot() +
  geom_ribbon(data = lmer_bray_total_predictions, aes(x = year, ymin = bray_curtis_lmer_preds_lowerCI, ymax = bray_curtis_lmer_preds_upperCI), fill = "grey", alpha = 0.3) +
  geom_point(data = distances_dissimilarities_allyears,
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
                 fill = Survey_Name_Season), alpha = 0.4, size = 1, shape = 21, color = "white") +
  ...
  geom_line(data = lmer_bray_total_predictions, aes(x = year, y = bray_curtis_lmer_preds), color = "black") +
    scale_color_manual(values =  rep("black", times = length(unique(distances_dissimilarities_allyears$Survey_Name_Season))), name = "Survey Unit") +
  scale_fill_manual(values =  rep("black", times = length(unique(distances_dissimilarities_allyears$Survey_Name_Season))), guide = "none") +
  theme_classic() +
  lims(x = c(min(distances_dissimilarities_allyears[,year]),max(distances_dissimilarities_allyears[,year]))
       ) +
  xlab("Year") +
...
points_wavylines_bray_total_year_reduced_gam_individual_facet_1_24 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears[Survey_Name_Season  %in% all_survey_names[c(1:24)]],
             aes(x = year,
...
aes(x = year, ymin=bray_glm_mod_fit-bray_glm_mod_fit_SE, ymax=bray_glm_mod_fit+bray_glm_mod_fit_SE), alpha=0.5) + #add standard error
    geom_smooth(data = distances_dissimilarities_allyears[Survey_Name_Season  %in% all_survey_names[c(1:24)]],
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
                 color = Survey_Name_Season,
...
#second half
distances_dissimilarities_allyears_second <- distances_dissimilarities_allyears[Survey_Name_Season  %in% all_survey_names[c(25:42)]]
distances_dissimilarities_allyears_second <- setorder(distances_dissimilarities_allyears_second,Survey_Name_Season)
...
points_wavylines_bray_total_year_reduced_gam_individual_facet_25_42 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears_second,
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean), alpha = 0.7) +
...
aes(x = year, ymin=bray_glm_mod_fit-bray_glm_mod_fit_SE, ymax=bray_glm_mod_fit+bray_glm_mod_fit_SE), alpha=0.5) + #add standard error
  geom_smooth(data = distances_dissimilarities_allyears_second,
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
                 color = Survey_Name_Season,
                 fill = Survey_Name_Season), alpha = 0.4, method = "lm", linewidth = 0.5) +
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend_EBSonly <- ggplot() +
  geom_point(data = na.omit(distances_dissimilarities_allyears[survey_unit == "EBS"], cols = "year_adj"),
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend_seus_summer <- ggplot() +
  geom_point(data = na.omit(distances_dissimilarities_allyears[survey_unit == "SEUS-summer"], cols = "year_adj"),
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
                 fill = Survey_Name_Season), size = 2, shape = 21, color = "white") +
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend_NZ_WCSI_SEUS_summeronly <- ggplot() +
  geom_point(data = na.omit(distances_dissimilarities_allyears[survey_unit %in% c("NZ-WCSI","SEUS-summer")], cols = "year_adj"),
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
                 fill = Survey_Name_Season), size = 2, shape = 21, color = "white") +
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend_NZ_WCSI_SEUS_summer_NEUSfallonly <- ggplot() +
  geom_point(data = na.omit(distances_dissimilarities_allyears[survey_unit %in% c("NZ-WCSI","SEUS-summer", "NEUS-Fall")], cols = "year_adj"),
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
                 fill = Survey_Name_Season), size = 2, shape = 21, color = "white") +
...
for (i in 1:length(all_survey_names)) {
  GAM_LM_model_comparisons_single <- compare_models(distances_dissimilarities_allyears[Survey_Name_Season == all_survey_names[i]])
