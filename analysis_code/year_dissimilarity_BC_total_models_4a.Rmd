...
bray_curtis_total_coefs <- data.table(summary(bray_curtis_total_lm)$coefficients)
  ...
  #hex by trend
  bray_curtis_total_coefs.r[,trend_color := ifelse(homogenizing == 1, "#e7ac5b", ifelse(differentiating == 1,"#91c874","#75608a"))]
...
jaccard_total_coefs <- data.table(summary(jaccard_total_lm)$coefficients)
  ...
  #hex by trend
  jaccard_total_coefs.r[,trend_color_jaccard := ifelse(homogenizing == 1, "#e7ac5b", ifelse(differentiating == 1,"#91c874","#75608a"))]
...
bray_curtis_balanced_coefs <- data.table(summary(bray_curtis_balanced_lm)$coefficients)
  ...
  #hex by trend
  bray_curtis_balanced_coefs.r[,trend_color_bray_curtis_balanced := ifelse(homogenizing == 1, "#e7ac5b", ifelse(differentiating == 1,"#91c874","#75608a"))]
...
bray_curtis_gradient_coefs <- data.table(summary(bray_curtis_gradient_lm)$coefficients)
  ...
  #hex by trend
  bray_curtis_gradient_coefs.r[,trend_color_bray_curtis_gradient := ifelse(homogenizing == 1, "#e7ac5b", ifelse(differentiating == 1,"#91c874","#75608a"))]
...
#wide to long
dissim_long <- melt(distances_dissimilarities_allyears.r, id.vars = c("year"), variable.name = "β-diversity metric")

dissim_long[,`β-diversity metric` := factor(`β-diversity metric`, levels = c( "bray_curtis_dissimilarity_total_mean","bray_curtis_dissimilarity_balanced_mean","bray_curtis_dissimilarity_gradient_mean", "jaccard_dissimilarity_total_mean"))]

BC_dissimilarity_components <- ggplot() +
  geom_point(data = dissim_long[`β-diversity metric` != "jaccard_dissimilarity_total_mean",], aes(x = year, y = value, color = `β-diversity metric`), alpha = 0.2, size = 1) +
...
Jaccard_dissimilarity <- ggplot() +
  geom_point(data = dissim_long[`β-diversity metric` == "jaccard_dissimilarity_total_mean"], aes(x = year, y = value), alpha = 0.2, size = 1) +
...
(jaccard_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL <- ggplot(data = jaccard_total_coefs.r, aes(x = reorder(Survey_Name_Season, survey_unit_coefficient) , y = survey_unit_coefficient), label = Survey_Name_Season) +
    geom_errorbar(aes(ymin = lwr, ymax = upr), fill = "grey", width = 0) + #add confidence intervals
  geom_point(aes(size = years_sampled, color = Directional_Change_sig), stat = 'identity') +
  scale_color_manual(values = c("#73BA4D","#E0962C","#75608a"), name = "β-diversity trend\nTotal Jaccard dissimilarity", guide="none") +
  scale_size_binned(range = c(1,8), name = "Survey period length\n(years)") +
  geom_hline(yintercept = 0) +
  #scale_y_continuous(breaks = seq(-0.005, 0.0075, by = 0.0025), labels = c("-0.005","","0", "", "0.005",  "")) +
  xlab("Survey unit") +
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend <- ggplot() +
  ...
  theme_classic() +
  lims(x = c(min(distances_dissimilarities_allyears[,year]),max(distances_dissimilarities_allyears[,year])),y = c(0.49,0.95)
       ) +
  xlab("Year") +
ylab("β-diversity") +
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend_colorbytrend_squigglesonly <- ggplot() +
  #points by trend
  geom_point(data = distances_dissimilarities_allyears, cols = "year_adj",
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend_colorbytrend_squigglesonly

ggsave(points_wavylines_bray_total_year_reduced_gam_colorbytrend_colorbytrend_squigglesonly, path = here::here("figures"), filename ="points_wavylines_bray_total_year_reduced_gam_colorbytrend_colorbytrend_squigglesonly.jpg", height = 6, width = 11, unit = "in")
```
...
BC_total_GAM_LM_merge_legend_colorbytrend <- ggdraw(xlim = c(0, 40.5), ylim = c(0, 21)) +
    draw_plot(points_wavylines_bray_total_year_reduced_gam_colorbytrend_colorbytrend + theme(axis.text = element_text(size = 8)),
...
BC_balanced_GAM_LM_merge_legend_colorbytrend <- ggdraw(xlim = c(0, 40.5), ylim = c(0, 21)) +
    draw_plot(points_wavylines_bray_curtis_balanced_year_reduced_gam_colorbytrend + theme(axis.text = element_text(size = 8)),
                                         x = 1, y = 1, width = 20, height = 20) +
...
BC_gradient_GAM_LM_merge_legend_colorbytrend <- ggdraw(xlim = c(0, 40.5), ylim = c(0, 21)) +
    draw_plot(points_wavylines_bray_curtis_gradient_year_reduced_gam_colorbytrend + theme(axis.text = element_text(size = 8)),
                                         x = 1, y = 1, width = 20, height = 20) +
...
jaccard_total_GAM_LM_merge_legend_colorbytrend <- ggdraw(xlim = c(0, 40.5), ylim = c(0, 21)) +
    draw_plot(points_wavylines_jaccard_total_year_reduced_gam_colorbytrend + theme(axis.text = element_text(size = 8)),
                                         x = 1, y = 1, width = 20, height = 20) +
...
GAM_LM_model_comparisons.f <- GAM_LM_model_comparisons[,c(1,3:5)]

colnames(GAM_LM_model_comparisons.f) <- c("Survey","Model comparison", "GAM AICc","LM AICc")
...
# Fit GAM model with "fs" basis and single smooth term
  ...
  return(round(st_dev_pred_dissim,4))
}
...
fwrite(GAM_lm_SD, here::here("output","GAM_lm_SD.csv"))
...
BC_total_model_coefs_reduced_length.unique.alpha[,trend_color := ifelse(Directional_Change_sig == "Homogenization", "#e7ac5b", ifelse(Directional_Change_sig == "Differentiation","#91c874","#75608a"))]
