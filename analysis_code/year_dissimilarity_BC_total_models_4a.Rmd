...
jaccard_total_gam <- gam(annual_dissimilarity_value ~ year + s(survey_unit, year, bs = "fs", m = 1),#random smooth
                            data = distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total"])

```


Bray Curtis for abundance
```{r bray curtis gams}
bray_curtis_total_gam <- gam(annual_dissimilarity_value ~ year + s(survey_unit, year, bs = "fs", m = 1),#random smooth
                            data = distances_dissimilarities_allyears[dissimilarity_metric == "bray_curtis_dissimilarity_total_abundance"])

```

Bray Curtis for relative
```{r bray curtis gams}
bray_curtis_total_relative_gam <- gam(annual_dissimilarity_value ~ year + s(survey_unit, year, bs = "fs", m = 1),#random smooth
                            data = distances_dissimilarities_allyears[dissimilarity_metric == "bray_curtis_dissimilarity_total_relative_abundance"])
...
bray_curtis_total_lme <- lme(annual_dissimilarity_value ~ year_adj, random = (~1 + year_adj|survey_unit),data = distances_dissimilarities_allyears[dissimilarity_metric == "bray_curtis_dissimilarity_total_abundance"])

#but also run with lmer for confint
#total
bray_curtis_total_lmer <- lmer(annual_dissimilarity_value ~ year_adj + (1 + year_adj|survey_unit),data = distances_dissimilarities_allyears[dissimilarity_metric == "bray_curtis_dissimilarity_total_abundance"]) #FYI doesn't converge

summary(bray_curtis_total_lme)
anova(bray_curtis_total_lme)

#Jaccard total with lme
jaccard_total_lme <- lme(annual_dissimilarity_value ~ year_adj, random = (~1 + year_adj|survey_unit),data = distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total"])

#Jaccard total with lmer
jaccard_total_lmer <- lmer(annual_dissimilarity_value ~ year_adj + (1 + year_adj|survey_unit),data = distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total"]) #FYI doesn't converge

summary(jaccard_total_lme)
anova(jaccard_total_lme)

#EXPORT BRAY CURTIS COEFFICIENTS
...
bray_curtis_total_lm <- lm(annual_dissimilarity_value ~ year * survey_unit,data = distances_dissimilarities_allyears[dissimilarity_metric == "bray_curtis_dissimilarity_total_abundance"])

#populate table of coefficients
bray_curtis_total_coefs <- data.table(summary(bray_curtis_total_lm)$coefficients)
  ...
  #limit to interactions only (check this if there are any model changes!) row 2 and rows 24:44
  bray_curtis_total_coefs.r <- bray_curtis_total_coefs[c(2,26:48),]
  ...
  bray_curtis_total_coefs.r[2:24,survey_unit_coefficient := (AI_estimate + Estimate)]
  ...
  #hex by trend
  bray_curtis_total_coefs.r[,trend_color_bray_curtis := ifelse(homogenizing == 1,"#FFC109", ifelse(differentiating == 1,"#D81C60","#1E88E5"))]
  ...
  #delete rows of surveys without abundance data
    bray_curtis_total_coefs.r <-     bray_curtis_total_coefs.r[complete.cases(  bray_curtis_total_coefs.r),]
  
  color_link_trend_bray_curtis <- bray_curtis_total_coefs.r[,.(survey_unit,Survey_Name_Season, hex,  trend_color_bray_curtis)]

```

Same, but for RELATIVE abundance
```{r linear model relative abundance Bray}

bray_curtis_total_relative_lm <- lm(annual_dissimilarity_value ~ year * survey_unit,data = distances_dissimilarities_allyears[dissimilarity_metric == "bray_curtis_dissimilarity_total_relative_abundance"])

bray_curtis_total_relative_coefs <- data.table(summary(bray_curtis_total_relative_lm)$coefficients)
  bray_curtis_total_relative_coefs[,var := rownames(summary(bray_curtis_total_relative_lm)$coefficients)]
  
  #limit to interactions only (check this if there are any model changes!) row 2 and rows 24:44
  bray_curtis_total_relative_coefs.r <- bray_curtis_total_relative_coefs[c(2,26:48),]
  
  #adjust survey unit name by deleting beginning of string
  bray_curtis_total_relative_coefs.r[,survey_unit := substr(var, 17, str_length(var))][var == "year",survey_unit := "AI"]
  
  #calculate interaction coefficients
  AI_estimate <- bray_curtis_total_relative_coefs.r[1,Estimate]
  bray_curtis_total_relative_coefs.r[1,survey_unit_coefficient := AI_estimate]
  bray_curtis_total_relative_coefs.r[2:24,survey_unit_coefficient := (AI_estimate + Estimate)]
  
  #homogenizing vs differentiating
  bray_curtis_total_relative_coefs.r[,differentiating := ifelse((`Std. Error`< abs(survey_unit_coefficient) & survey_unit_coefficient > 0),1,0)][,homogenizing := ifelse((`Std. Error`< abs(survey_unit_coefficient) & survey_unit_coefficient < 0),1,0)]
  
  #lower and upper CI
    bray_curtis_total_relative_coefs.r[,lwr := survey_unit_coefficient-`Std. Error`][,upr:= survey_unit_coefficient+`Std. Error`]
    
    #save as output
    fwrite(bray_curtis_total_relative_coefs.r, here::here("output","bray_curtis_total_relative_coefs.r.csv"))
  
  bray_curtis_total_relative_coefs_sum <- data.table(differentiating = sum(  bray_curtis_total_relative_coefs.r$differentiating), homogenizing = sum(  bray_curtis_total_relative_coefs.r$homogenizing))
  
  #hex by trend
  bray_curtis_total_relative_coefs.r[,trend_color_bray_curtis_relative := ifelse(homogenizing == 1,"#FFC109", ifelse(differentiating == 1,"#D81C60","#1E88E5"))]
  
  #link to color link
  bray_curtis_total_relative_coefs.r <- color_link[bray_curtis_total_relative_coefs.r, on = "survey_unit"]
  
  #delete rows of surveys without abundance data
    bray_curtis_total_relative_coefs.r <-     bray_curtis_total_relative_coefs.r[complete.cases(  bray_curtis_total_relative_coefs.r),]
  
  color_link_relative_trend_bray_curtis <- bray_curtis_total_relative_coefs.r[,.(survey_unit,Survey_Name_Season, hex,  trend_color_bray_curtis_relative)]
...
jaccard_total_lm <- lm(annual_dissimilarity_value ~ year * survey_unit,data = distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total"])

#alternative, what if we use GLS (allows us to include temporal autocorrelation term)
jaccard_total_gls <- gls(annual_dissimilarity_value ~ year * survey_unit,correlation = corAR1(form=~year | survey_unit),data = distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total"])

#compare these two models
AICc(jaccard_total_lm, jaccard_total_gls)

AICc(jaccard_total_gls)-AICc(jaccard_total_lm) #615.1913
...
jaccard_total_coefs <- data.table(summary(jaccard_total_lm)$coefficients)
  ...
  jaccard_total_coefs.r <- jaccard_total_coefs[c(2,36:68),]
  ...
  jaccard_total_coefs.r[2:34,survey_unit_coefficient := (AI_estimate + Estimate)]
...
Bray Curtis true abundance
```{r lollipop figure 2b main text bray curtis true abundance}

###############
#FOR LINEAR MODEL INSTEAD OF LINEAR MIXED EFFECTS as of fall 2023
###############

bray_curtis_total_coefs.r[,Directional_Change_sig := ifelse(differentiating > 0, "Differentiation",ifelse(homogenizing > 0 , "Homogenization", "No β-diversity trend"))]
years_sampled <- unique(distances_dissimilarities_allyears[,.(survey_unit, years_sampled)])
bray_curtis_total_coefs.r <- years_sampled[bray_curtis_total_coefs.r, on = "survey_unit"]
...
```{r lollipop figure 2b main text bray curtis relative abundance}

bray_curtis_total_relative_coefs.r[,Directional_Change_sig := ifelse(differentiating > 0, "Differentiation",ifelse(homogenizing > 0 , "Homogenization", "No β-diversity trend"))]
years_sampled <- unique(distances_dissimilarities_allyears[,.(survey_unit, years_sampled)])
bray_curtis_total_relative_coefs.r <- years_sampled[bray_curtis_total_relative_coefs.r, on = "survey_unit"]

(BC_relative_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL <- ggplot(data = bray_curtis_total_relative_coefs.r, aes(x = reorder(Survey_Name_Season, survey_unit_coefficient) , y = survey_unit_coefficient), label = Survey_Name_Season) +
    geom_errorbar(aes(ymin = lwr, ymax = upr), fill = "grey", width = 0) + #add confidence intervals
  geom_point(aes(size = years_sampled, color = Directional_Change_sig), stat = 'identity') +
  scale_color_manual(values = c("#D81C60","#FFC109","#1E88E5"), name = "Dissimilarity trend", guide="none") +
  scale_size_binned(range = c(1,8), name = "Survey period length\n(years)") +
...
ggsave(BC_relative_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL, 
       path = here::here("figures"), filename = "BC_relative_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL.jpg", height = 7, width = 7)

#plot for legend
directional_change_legend_plot_colorbytrend <- BC_relative_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL + 
  theme(legend.position = "right", legend.background = element_rect(fill= "transparent"), 
         legend.text = element_text(size = 15), legend.title = element_text(size = 16)) +
  guides(colour = guide_legend(override.aes = list(size=6)), size = "none")
```
...
```{r}
jaccard_total_coefs.r[,jaccard_coefficient := survey_unit_coefficient][,Directional_Change_sig_jaccard := Directional_Change_sig]
...
#add bray metrics
bray_curtis_total_coefs.r[,bray_coefficient := survey_unit_coefficient][,Directional_Change_sig_bray := Directional_Change_sig]
bray_curtis_total_coefs.r.r <- bray_curtis_total_coefs.r[,.(survey_unit, bray_coefficient, Directional_Change_sig_bray, trend_color_bray_curtis)]
all_total_coefs <- all_total_coefs[bray_curtis_total_coefs.r.r, on = "survey_unit"]

#add relative bray metrics
bray_curtis_total_relative_coefs.r[,bray_relative_coefficient := survey_unit_coefficient][,Directional_Change_sig_bray_relative := Directional_Change_sig]
bray_curtis_total_relative_coefs.r.r <- bray_curtis_total_relative_coefs.r[,.(survey_unit, bray_relative_coefficient, Directional_Change_sig_bray_relative, trend_color_bray_curtis_relative)]
all_total_coefs <- all_total_coefs[bray_curtis_total_relative_coefs.r.r, on = "survey_unit"]
...
(Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_comparison <- ggplot(data = all_total_coefs, aes(x = reorder(Survey_Name_Season, jaccard_coefficient)), label = Survey_Name_Season) +
  ...
  geom_point(aes(y = bray_coefficient, color = Directional_Change_sig_bray), stat = 'identity', shape = 15, alpha = 0.5,size = 5) +
  geom_point(aes(y = bray_relative_coefficient, color = Directional_Change_sig_bray_relative), stat = 'identity', shape = 17, alpha = 0.5,size = 5) +
...
year_survey_unit_expand_bray.dt <- year_survey_unit_expand.dt[color_link_trend_bray_curtis, on = "survey_unit"]
...
Bray Curtis Total based on abundance input
```{r}
#for plotting, get model as predictions
bray_curtis_total_gam_predictions <- predict(bray_curtis_total_gam, se.fit = TRUE, newdata = year_survey_unit_expand_bray.dt)

#merge into table
year_survey_unit_expand_bray.dt[,bray_glm_mod_fit := bray_curtis_total_gam_predictions$fit][,bray_glm_mod_fit_SE := bray_curtis_total_gam_predictions$se.fit]
...
distances_dissimilarities_allyears_bray_curtis_abun <- distances_dissimilarities_allyears[color_link_trend_bray_curtis, on = "survey_unit"]
distances_dissimilarities_allyears_bray_curtis_abun <- distances_dissimilarities_allyears_bray_curtis_abun[dissimilarity_metric == "bray_curtis_dissimilarity_total_abundance"]
#and again for jaccard
distances_dissimilarities_allyears_jaccard <- distances_dissimilarities_allyears[color_link_trend_jaccard, on = "survey_unit"]
distances_dissimilarities_allyears_jaccard<- distances_dissimilarities_allyears_jaccard[dissimilarity_metric == "jaccard_dissimilarity_total"]

#change survey_unit to factor
distances_dissimilarities_allyears_bray_curtis_abun[,survey_unit := factor(survey_unit)]
year_survey_unit_expand_bray.dt[,survey_unit := factor(survey_unit)]

#check that survey_unit in alpha order
sort(unique(distances_dissimilarities_allyears_bray_curtis_abun$survey_unit)) == levels(distances_dissimilarities_allyears_bray_curtis_abun$survey_unit)
sort(unique(year_survey_unit_expand_bray.dt$survey_unit)) == levels(year_survey_unit_expand_bray.dt$survey_unit)

#get list of colors in the right order
trend_colors_bray.dt <- unique(distances_dissimilarities_allyears_bray_curtis_abun[dissimilarity_metric == "bray_curtis_dissimilarity_total_abundance",.(survey_unit, Survey_Name_Season, trend_color_bray_curtis)])

#make sure it matches year_survey_unit_expand.dt
all(trend_colors_bray.dt == unique(year_survey_unit_expand_bray.dt[,.(survey_unit, Survey_Name_Season, trend_color_bray_curtis)]))

setorder(trend_colors_bray.dt, survey_unit)
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend <- ggplot() +
  ...
  #points by trend
  geom_point(data = distances_dissimilarities_allyears_bray_curtis_abun[dissimilarity_metric =="bray_curtis_dissimilarity_total_abundance"], cols = "year_adj",
             aes(x = year,
  ...
  #squiggly lines by survey unit based GAM and colored by trend
    geom_line(data = year_survey_unit_expand_bray.dt, cols = "year_adj",
             aes(x = year,
                 y = bray_glm_mod_fit,
  ...
  geom_ribbon(data = year_survey_unit_expand_bray.dt, cols = "year_adj", 
  ...
  #set color by survey
    scale_color_manual(values = trend_colors_bray.dt$trend_color, name = "Survey Unit") +
  scale_fill_manual(values =trend_colors_bray.dt$trend_color, guide = "none") +
  theme_classic() +
  xlab("Year") +
...
#link distances dissimilarities with trend color hex
distances_dissimilarities_allyears_jaccard <- distances_dissimilarities_allyears[color_link_trend_jaccard, on = "survey_unit"]
distances_dissimilarities_allyears_jaccard <- distances_dissimilarities_allyears_jaccard[dissimilarity_metric == "jaccard_dissimilarity_total_abundance"]
#and again for jaccard
distances_dissimilarities_allyears_jaccard <- distances_dissimilarities_allyears[color_link_trend_jaccard, on = "survey_unit"]
distances_dissimilarities_allyears_jaccard<- distances_dissimilarities_allyears_jaccard[dissimilarity_metric == "jaccard_dissimilarity_total"]

#change survey_unit to factor
distances_dissimilarities_allyears_jaccard[,survey_unit := factor(survey_unit)]
year_survey_unit_expand_jaccard.dt[,survey_unit := factor(survey_unit)]

#check that survey_unit in alpha order
sort(unique(distances_dissimilarities_allyears_jaccard$survey_unit)) == levels(distances_dissimilarities_allyears_jaccard$survey_unit)
sort(unique(year_survey_unit_expand_jaccard.dt$survey_unit)) == levels(year_survey_unit_expand_jaccard.dt$survey_unit)

#get list of colors in the right order
trend_colors_jaccard.dt <- unique(distances_dissimilarities_allyears_jaccard[dissimilarity_metric == "jaccard_dissimilarity_total",.(survey_unit, Survey_Name_Season, trend_color_jaccard)])
...
points_wavylines_jaccard_total_year_reduced_gam_colorbytrend <- ggplot() +
  ...
  #points by trend
  geom_point(data = distances_dissimilarities_allyears[dissimilarity_metric =="jaccard_dissimilarity_total"], cols = "year_adj",
             aes(x = year,
                 y = annual_dissimilarity_value,
                 fill = survey_unit), alpha = 0.4, size = 1, shape = 21, color = "white") +
  #squiggly lines by survey unit based GAM and colored by trend
  ...
  theme_classic() +
  lims(x = c(min(distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total",year]),max(distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total",year]))
       ) +
  xlab("Year\n") +
ylab("β-diversity\n(Jaccard dissimilarity)") +
  theme(legend.position = "null", axis.text = element_text(size = 15), axis.title = element_text(size = 15))
...
#list of surveys
survey_unit.list_jaccard <- levels(factor(distances_dissimilarities_allyears_jaccard$survey_unit))
...
for (i in 1:length(survey_unit.list_jaccard)) {
  GAM_LM_model_comparisons_jaccard_total_single <- compare_models(distances_dissimilarities_allyears_jaccard[survey_unit == survey_unit.list_jaccard[i],])
...
for (i in 1:length(survey_unit.list_jaccard)) {
  st_dev_pred_dissim <- GAM_outputs(distances_dissimilarities_allyears_jaccard[survey_unit == survey_unit.list_jaccard[i]])
...
```{r compare lms to gams for each region bray curtis}

#list of surveys
survey_unit.list_bray_curtis <- levels(factor(distances_dissimilarities_allyears_bray_curtis_abun$survey_unit))

# Function to compare GAM and linear model fits
#see funciton defined in previous chunk


############## Compare models for each survey name and bind the results
GAM_LM_model_comparisons_bray_curtis_total <- data.table()

for (i in 1:length(survey_unit.list_bray_curtis)) {
  GAM_LM_model_comparisons_bray_curtis_total_single <- compare_models(distances_dissimilarities_allyears_bray_curtis_abun[survey_unit == survey_unit.list_bray_curtis[i],])
  GAM_LM_model_comparisons_bray_curtis_total <- rbind(GAM_LM_model_comparisons_bray_curtis_total, GAM_LM_model_comparisons_bray_curtis_total_single)
}

GAM_LM_model_comparisons_bray_curtis_total

#round to 2 significant figures

#names of numeric columns
numeric_cols <- names(GAM_LM_model_comparisons_bray_curtis_total)[sapply(GAM_LM_model_comparisons_bray_curtis_total, is.numeric)]

# Apply signif() only to numeric columns
GAM_LM_model_comparisons_bray_curtis_total[, (numeric_cols) := lapply(.SD, function(x) if (is.numeric(x)) signif(x, digits = 2) else x), .SDcols = numeric_cols]

GAM_LM_model_comparisons_bray_curtis_total.f <- GAM_LM_model_comparisons_bray_curtis_total[,-2]

colnames(GAM_LM_model_comparisons_bray_curtis_total.f) <- c("Survey","Model comparison", "GAM AICc","GAM p-value","LM AICc", "LM p-value")

fwrite(GAM_LM_model_comparisons_bray_curtis_total.f, here::here("output","GAM_LM_model_comparisons_bray_curtis_total.csv"))


##################generate predicted GAM values
#see funciton defined in previous chunk

predicted_values_bray_curtis <- data.table(survey_unit = survey_unit.list_bray_curtis, SD_pred_dissim = as.numeric())

for (i in 1:length(survey_unit.list_bray_curtis)) {
  st_dev_pred_dissim <- GAM_outputs(distances_dissimilarities_allyears_bray_curtis_abun[survey_unit == survey_unit.list_bray_curtis[i]])
  predicted_values_bray_curtis[i,"SD_pred_dissim"] <- st_dev_pred_dissim
}

#merge witih GAM vs LM

GAM_lm_SD_bray_curtis_total <- predicted_values_bray_curtis[GAM_LM_model_comparisons_bray_curtis_total, on = "survey_unit"]

GAM_lm_SD_bray_curtis_total.f <- GAM_lm_SD_bray_curtis_total[,.(Survey, SD_pred_dissim, aicc_gam, gam_p, aicc_lm, lm_p, comparison)]

colnames(GAM_lm_SD_bray_curtis_total.f) <- c("Survey","SD over time of predicted GAM values", "GAM AICc","GAM p-value","LM AICc", "LM p-value","Model comparison")

View(GAM_lm_SD_bray_curtis_total.f)

fwrite(GAM_lm_SD_bray_curtis_total.f, here::here("output","GAM_lm_SD_bray_curtis_total.f.csv"))
...
distances_dissimilarities_allyears_jaccard[,Survey_Name_Season := factor(Survey_Name_Season, levels = trend_colors_jaccard.dt$Survey_Name_Season, ordered = T)]

#change year to integer
distances_dissimilarities_allyears_jaccard[,year:=as.integer(year)]

#split into 2 and use facet
#first 17
points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_individual_facet_1_20 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears_jaccard[survey_unit  %in% all_survey_names[c(1:20)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value), alpha = 0.3) +

    geom_smooth(data = distances_dissimilarities_allyears_jaccard[survey_unit  %in% all_survey_names[c(1:20)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value,
                 color = survey_unit,
...
points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_individual_facet_21_34 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears_jaccard[survey_unit  %in% all_survey_names[c(21:34)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value), alpha = 0.3) +

    geom_smooth(data = distances_dissimilarities_allyears_jaccard[survey_unit  %in% all_survey_names[c(21:34)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value,
                 color = survey_unit,
                 fill = survey_unit), method = "lm", linewidth = 0.5) +
...
Plot each bray_curtis total independently 

Note that this is for independent linear models and independent generalized additive models, NOT models built for all surveys at once
```{r plot each independently bray_curtis}
#plot each independently for supplement
#all survey names = 
all_survey_names <- color_link_trend_bray_curtis$survey_unit

#set order of levels of Survey Name Season by survey unit
distances_dissimilarities_allyears_bray_curtis_abun[,Survey_Name_Season := factor(Survey_Name_Season, levels = trend_colors_bray.dt$Survey_Name_Season, ordered = T)]

#change year to integer
distances_dissimilarities_allyears_bray_curtis_abun[,year:=as.integer(year)]

#split into 2 and use facet
#first 17
points_wavylines_bray_curtis_total_year_reduced_gam_colorbytrend_individual_facet_1_20 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears_bray_curtis_abun[survey_unit  %in% all_survey_names[c(1:20)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value), alpha = 0.3) +

    geom_smooth(data = distances_dissimilarities_allyears_bray_curtis_abun[survey_unit  %in% all_survey_names[c(1:20)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value,
                 color = survey_unit,
                 fill = survey_unit), method = "lm", linewidth = 0.5) +
  geom_smooth(data = distances_dissimilarities_allyears_bray_curtis_abun[survey_unit %in% all_survey_names[c(1:20)]],
              aes(x = as.integer(year),
                  y = annual_dissimilarity_value), method = "gam", linewidth = 0.5, formula = y ~ s(x, bs = "fs", m = 1), color = "black") +
    scale_color_manual(values =    color_link_trend_bray_curtis$trend_color[1:20], name = "Survey Unit") +
    scale_fill_manual(values =    color_link_trend_bray_curtis$trend_color[1:20], name = "Survey Unit") +
  scale_x_continuous(breaks = ~ axisTicks(., log = FALSE)) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 7)) +
  xlab("Year") +
...
ggsave(points_wavylines_bray_curtis_total_year_reduced_gam_colorbytrend_individual_facet_1_20, path =  here::here("figures"), filename = "points_wavylines_bray_curtis_total_year_reduced_gam_colorbytrend_individual_facet_1_20.png", height = 12, width = 9.5)

#second portion
points_wavylines_bray_curtis_total_year_reduced_gam_colorbytrend_individual_facet_21_34 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears_bray_curtis_abun[survey_unit  %in% all_survey_names[c(21:34)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value), alpha = 0.3) +

    geom_smooth(data = distances_dissimilarities_allyears_bray_curtis_abun[survey_unit  %in% all_survey_names[c(21:34)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value,
                 color = survey_unit,
                 fill = survey_unit), method = "lm", linewidth = 0.5) +
  geom_smooth(data = distances_dissimilarities_allyears_bray_curtis_abun[survey_unit %in% all_survey_names[c(21:34)]],
              aes(x = as.integer(year),
                  y = annual_dissimilarity_value), method = "gam", linewidth = 0.5, formula = y ~ s(x, bs = "fs", m = 1), color = "black") +
    scale_color_manual(values =    color_link_trend_bray_curtis$trend_color[21:34], name = "Survey Unit") +
    scale_fill_manual(values =    color_link_trend_bray_curtis$trend_color[21:34], name = "Survey Unit") +
  scale_x_continuous(breaks = ~ axisTicks(., log = FALSE)) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 7)) +
  xlab("Year") +
...
ggsave(points_wavylines_bray_curtis_total_year_reduced_gam_colorbytrend_individual_facet_21_34, path =  here::here("figures"), filename = "points_wavylines_bray_curtis_total_year_reduced_gam_colorbytrend_individual_facet_21_34.png", height = 3, width = 5)
...
model_coefs_reduced_length[,Directional_Change_sig := ifelse(year_adj > 0 & significant == T, "Differentiation",ifelse(year_adj < 0 & significant == T, "Homogenization", "No β-diversity trend"))]
...
BC_total_model_coefs_reduced_length.unique.alpha[,trend_color := ifelse(Directional_Change_sig == "Homogenization", "#D81C60", ifelse(Directional_Change_sig == "Differentiation","#FFC109","#D81C60"))]
