---
title: "Calculating Dissimilarity Metrics Across Space and Time for top 90% of species"
output: html_notebook
---
This code is Script 3b for Kitchel et al. TITLE manuscript.
...
Delete 10% of least abundant species by biomass (across whole time series)
```{r}
...
FishGlob_clean.noNA <- name_helper[FishGlob_clean.noNA, on = "survey_unit"]
FishGlob_clean.noNA_15perc_excluded <- name_helper[FishGlob_clean.noNA_15perc_excluded, on = "survey_unit"]
```
...
for (i in 1:length(all_survey_names)) {
  survey_unit_spp_list[i] <- list(sort(unique(FishGlob_clean.noNA[Survey_Name_Season == all_survey_names[i], accepted_name])))
}

names(survey_unit_spp_list) <- all_survey_names

#spp excluded
survey_unit_spp_list_excluded <- list()
for (i in 1:length(all_survey_names)) {
  survey_unit_spp_list_excluded[i] <- list(sort(unique(FishGlob_clean.noNA_15perc[Survey_Name_Season == all_survey_names[i] & exclude_spp == T, accepted_name])))
}

names(survey_unit_spp_list_excluded) <- paste0(all_survey_names,"_15exclude")

survey_unit_spp_list_all <- c(survey_unit_spp_list,survey_unit_spp_list_excluded)
...
For now, do NA.RM = T, but at some point I need to figure out why we have NAs to begin with in distances_dissimilarities_allyears_15perc_excluded
...
distances_dissimilarities_allyears_15perc_excluded[, year_f := as.factor(year)]

#in case you didn't make it earlier
all_survey_units <- unique(distances_dissimilarities_allyears_15perc_excluded[, survey_unit])

for (i in 1:length(all_survey_units)) {
      distances_dissimilarities_allyears_15perc_excluded_subset <- distances_dissimilarities_allyears_15perc_excluded[season_survey == all_survey_units[i]]
      
    #jaccard similarity
#    box_plots_jaccard[[i]] <- ggplot(distances_dissimilarities_allyears_15perc_excluded_subset, aes(year_f,  #jaccard_dissimilarity_turnover)) +
...
#    box_plots_bray[[i]] <- ggplot(distances_dissimilarities_allyears_15perc_excluded_subset, aes(year_f, bray_curtis_dissimilari#ty_balanced)) +
#      geom_boxplot(outlier.shape = NA,  lwd = 0.2) +
...
#      theme(axis.text.x = element_text(angle = 90))
    ...
    avg_jaccard_mod <- lm(data = distances_dissimilarities_allyears_15perc_excluded_subset,  formula = jaccard_dissimilarity_turnover ~ as.numeric(year))
    
    jaccard_model_outputs[i,  "survey"] <- distances_dissimilarities_allyears_15perc_excluded_subset[1,  survey]
    jaccard_model_outputs[i,  "season"] <- distances_dissimilarities_allyears_15perc_excluded_subset[1,  season]
    jaccard_model_outputs[i,  "season_survey"] <- distances_dissimilarities_allyears_15perc_excluded_subset[1,  season_survey]
    ...
    avg_bray_mod <- lm(data = distances_dissimilarities_allyears_15perc_excluded_subset,  formula = bray_curtis_dissimilarity_balanced ~ as.numeric(year))
    
    bray_model_outputs[i,  "survey"] <- distances_dissimilarities_allyears_15perc_excluded_subset[1,  survey]
    bray_model_outputs[i,  "season"] <- distances_dissimilarities_allyears_15perc_excluded_subset[1,  season]
    bray_model_outputs[i,  "season_survey"] <- distances_dissimilarities_allyears_15perc_excluded_subset[1,  season_survey]
...
```{r}
overall_mod <- gls(bray_curtis_dissimilarity_balanced ~ year + season_survey,  data = distances_dissimilarities_allyears_15perc_excluded)
```
...
```{r}
colnames(distances_dissimilarities_allyears_15perc_excluded)


distances_dissimilarities_allyears_15perc_excluded[, mean_jaccard := mean(jaccard_dissimilarity_turnover),  .(survey,  season,  year)]
distances_dissimilarities_allyears_15perc_excluded[, mean_bray := mean(bray_curtis_dissimilarity_balanced),  .(survey,  season,  year)]
```
