...
FishGlob_clean[survey_unit != "MEDITS",abundance_spp_sum := sum(wgt_cpue),.(accepted_name, survey_unit)][survey_unit != "MEDITS",abundance_sum := sum(wgt_cpue),survey_unit][survey_unit == "MEDITS",abundance_spp_sum := sum(num_cpue),.(accepted_name, survey_unit)][survey_unit == "MEDITS",abundance_sum := sum(num_cpue),survey_unit]



FishGlob_abundance_survey <- unique(FishGlob_clean[,.(survey_unit, accepted_name, abundance_spp_sum, abundance_sum)])
...
FishGlob_clean_15perc <- spp_survey_keep_key[FishGlob_clean, on = .(survey_unit, accepted_name)]

FishGlob_clean_15perc_excluded <- FishGlob_clean_15perc[exclude_spp == F,]

#drops 
nrow(FishGlob_clean)-nrow(FishGlob_clean_15perc_excluded)
#4528/2727007 rows (0.2% of observations)

saveRDS(FishGlob_clean_15perc_excluded, here::here("output","FishGlob_clean_15perc_excluded.Rds"))
```

Delete any tows without any observations
```{r}
FishGlob_clean_15perc_excluded[,tow_abundance_sum := ifelse(survey == "MEDITS", sum(num_cpue),sum(wgt_cpue)),.(haul_id)]
summary(FishGlob_clean_15perc_excluded$tow_abundance_sum)

tows_by_abundance <- unique(FishGlob_clean_15perc_excluded[,.(haul_id, tow_abundance_sum)])
tows_noabundance <- unique(FishGlob_clean_15perc_excluded[tow_abundance_sum == 0,.(haul_id, tow_abundance_sum, survey_unit, year)])

length(unique(FishGlob_clean_15perc_excluded[tow_abundance_sum == 0,haul_id])) 

FishGlob_clean_15perc_excluded <- FishGlob_clean_15perc_excluded[!(haul_id %in% tows_noabundance$haul_id)]
```
...
distances_dissimilarities_allyears_15perc_excluded <- data.table("bray_curtis_dissimilarity_balanced_mean" = numeric(),
...
for (i in 1:length(all_survey_units)) {
  ...
  FishGlob_clean_15perc_excluded_subset <- FishGlob_clean_15perc_excluded[survey_unit == all_survey_units[i], ]
  
  #map
  ####unique lat lon
  #add column with adjusted longitude for few surveys that cross dateline (NZ-CHAT and AI)
  FishGlob_clean_15perc_excluded_subset[,longitude_adj := ifelse((survey_unit %in% c("AI","NZ-CHAT") & longitude > 0),longitude-360,longitude)]

    lat_lon <- unique(FishGlob_clean_15perc_excluded_subset[, .(latitude,  longitude_adj)])

   #list years
  FishGlob_clean_15perc_excluded_subset[, year:= as.numeric(year)] #make numeric
  setorder(FishGlob_clean_15perc_excluded_subset,  year)
  years <- unique(FishGlob_clean_15perc_excluded_subset[, year])
  
  #haul id keys
  haul_ids <- unique(FishGlob_clean_15perc_excluded_subset[, haul_id])
  haul_ids_key <- data.table(haul_id = haul_ids,  key_ID = seq(1, length(haul_ids),  by = 1))
  
  
  #convert haul_ids to numeric key_ids
  FishGlob_clean_15perc_excluded_subset <- FishGlob_clean_15perc_excluded_subset[haul_ids_key,  on = "haul_id"]

          for (j in 1:length(years)) {
            reduced_year <- FishGlob_clean_15perc_excluded_subset[year == years[j], ]
          ...
          if(!(reduced_year[1, survey] == "MEDITS")) {
            ...
            reduced_year <- reduced_year[complete.cases(reduced_year[, wgt_cpue]), ]
              ...
              dissimilarities_full.r[, "survey" := FishGlob_clean_15perc_excluded_subset[1, survey]]
              ...
              #bind with full
              distances_dissimilarities_allyears_15perc_excluded <- rbind(distances_dissimilarities_allyears_15perc_excluded, dissimilarities_full.r)
