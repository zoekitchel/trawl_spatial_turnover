...
Script 4e for Kitchel et al. 2023 in prep taxonomic diversity manuscript.
...
bray_curtis_total_gam <- gam(bray_curtis_dissimilarity_total_mean ~ year + s(survey_unit, year, bs = "fs", m = 1),#random smooth
                            data = distances_dissimilarities_allyears_15perc_excluded)
...
bray_curtis_total_lme <- lme(bray_curtis_dissimilarity_total_mean ~ year_adj, random = (~1 + year_adj|survey_unit),data = distances_dissimilarities_allyears_15perc_excluded)

#but also run with lmer for confint
#total
bray_curtis_total_lmer <- lmer(bray_curtis_dissimilarity_total_mean ~ year_adj + (1 + year_adj|survey_unit),data = distances_dissimilarities_allyears_15perc_excluded)
...
bray_curtis_total_lm <- lm(bray_curtis_dissimilarity_total_mean ~ year * survey_unit,data = distances_dissimilarities_allyears_15perc_excluded)
...
bray_curtis_total_coefs <- data.table(summary(bray_curtis_total_lm)$coefficients)
  ...
  bray_curtis_total_coefs_15perc_exclude.r <- bray_curtis_total_coefs[c(2,44:84),]
  
  #adjust survey unit name by deleting beginning of string
  bray_curtis_total_coefs_15perc_exclude.r[,survey_unit := substr(var, 17, str_length(var))][var == "year",survey_unit := "AI"]
  
  #calculate interaction coefficients
  AI_estimate <- bray_curtis_total_coefs_15perc_exclude.r[1,Estimate]
  bray_curtis_total_coefs_15perc_exclude.r[1,survey_unit_coefficient := AI_estimate]
  bray_curtis_total_coefs_15perc_exclude.r[2:42,survey_unit_coefficient := (AI_estimate + Estimate)]
  
  #homogenizing vs differentiating
  bray_curtis_total_coefs_15perc_exclude.r[,differentiating := ifelse((`Std. Error`< abs(survey_unit_coefficient) & survey_unit_coefficient > 0),1,0)][,homogenizing := ifelse((`Std. Error`< abs(survey_unit_coefficient) & survey_unit_coefficient < 0),1,0)]
  
  #lower and upper CI
    bray_curtis_total_coefs_15perc_exclude.r[,lwr := survey_unit_coefficient-`Std. Error`][,upr:= survey_unit_coefficient+`Std. Error`]
    
    #save as output
    fwrite(bray_curtis_total_coefs_15perc_exclude.r, here::here("output","bray_curtis_total_coefs_15perc_exclude.r.csv"))
  
  bray_curtis_total_coefs_sum <- data.table(differentiating = sum(  bray_curtis_total_coefs_15perc_exclude.r$differentiating), homogenizing = sum(  bray_curtis_total_coefs_15perc_exclude.r$homogenizing))
  
  #hex by trend
  bray_curtis_total_coefs_15perc_exclude.r[,trend_color := ifelse(homogenizing == 1, "#D81C60", ifelse(differentiating == 1,"#FFC109","#1E88E5"))]
  
  #link to color link
  bray_curtis_total_coefs_15perc_exclude.r <- color_link[bray_curtis_total_coefs_15perc_exclude.r, on = "survey_unit"]
  
  color_link_trend <- bray_curtis_total_coefs_15perc_exclude.r[,.(survey_unit,Survey_Name_Season, hex,  trend_color)]
...
bray_curtis_total_coefs_15perc_exclude.r[,Directional_Change_sig := ifelse(differentiating > 0, "Differentiation",ifelse(homogenizing > 0 , "Homogenization", "No trend in\ndissimilarity"))]
years_sampled <- unique(distances_dissimilarities_allyears_15perc_excluded[,.(survey_unit, years_sampled)])
bray_curtis_total_coefs_15perc_exclude.r <- bray_curtis_total_coefs_15perc_exclude.r[years_sampled, on = "survey_unit"]

(BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_15perc_exclude <- ggplot(data = bray_curtis_total_coefs_15perc_exclude.r, aes(x = reorder(Survey_Name_Season, survey_unit_coefficient) , y = survey_unit_coefficient), label = Survey_Name_Season) +
...
ggsave(BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_15perc_exclude, 
       path = here::here("figures"), filename = "BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_15perc_exclude.jpg", height = 7, width = 7)

#plot for legend
directional_change_legend_plot_colorbytrend <- BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_15perc_exclude + 
...
distances_dissimilarities_allyears_15perc_excluded <- distances_dissimilarities_allyears_15perc_excluded[color_link_trend, on = "survey_unit"]

#change survey_unit to factor
distances_dissimilarities_allyears_15perc_excluded[,survey_unit := factor(survey_unit)]
year_survey_unit_expand.dt[,survey_unit := factor(survey_unit)]

#check that survey_unit in alpha order
sort(unique(distances_dissimilarities_allyears_15perc_excluded$survey_unit)) == levels(distances_dissimilarities_allyears_15perc_excluded$survey_unit)
...
trend_colors.dt <- unique(distances_dissimilarities_allyears_15perc_excluded[,.(survey_unit, Survey_Name_Season, trend_color)])
...
points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude <- ggplot() +
  ...
  #points by trend
  geom_point(data = distances_dissimilarities_allyears_15perc_excluded, cols = "year_adj",
             aes(x = year,
  ...
  theme_classic() +
        #add year breaks
  scale_x_continuous(breaks = c(1968,1980,1990,2000,2010,2022), limits = c(1966,2025)) +
  xlab("Year") +
ylab("β-diversity") +
  theme(legend.position = "null", axis.text = element_text(size = 15), axis.title = element_text(size = 15))

points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude

ggsave(points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude, path = here::here("figures"), filename ="points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude.jpg", height = 6, width = 11, unit = "in")


points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude_squigglesonly <- ggplot() +
  #points by trend
  geom_point(data = distances_dissimilarities_allyears_15perc_excluded, cols = "year_adj",
             aes(x = year,
                 y = bray_curtis_dissimilarity_total_mean,
  ...
  scale_fill_manual(values =trend_colors.dt$trend_color, guide = "none") +
  theme_classic() +
  lims(x = c(min(distances_dissimilarities_allyears_15perc_excluded[,year]),max(distances_dissimilarities_allyears_15perc_excluded[,year])),y = c(0.49,0.95)
       ) +
        #add year breaks
  scale_x_continuous(breaks = seq(min(distances_dissimilarities_allyears_15perc_excluded$year),max(distances_dissimilarities_allyears_15perc_excluded$year), by = 10)) +
  xlab("Year") +
ylab("β-diversity") +
  theme(legend.position = "null", axis.text = element_text(size = 15), axis.title = element_text(size = 15))

points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude_squigglesonly

ggsave(points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude_squigglesonly, path = here::here("figures"), filename ="points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude_squigglesonly.jpg", height = 6, width = 11, unit = "in")
```
...
######################################################
##################BCTOTAL
BC_total_GAM_LM_merge_legend_colorbytrend_15perc_exclude <- ggdraw(xlim = c(0, 40.5), ylim = c(0, 21)) +
    draw_plot(points_wavylines_bray_total_year_reduced_gam_colorbytrend_15perc_exclude + theme(axis.text = element_text(size = 15)),
                                         x = 1, y = 1, width = 20, height = 20) +
    draw_plot(BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_15perc_exclude +
...
ggsave(BC_total_GAM_LM_merge_legend_colorbytrend_15perc_exclude, path = here::here("figures"), filename = "BC_total_GAM_LM_merge_legend_colorbytrend_15perc_exclude.png", height = 8, width = 14, units = "in")
