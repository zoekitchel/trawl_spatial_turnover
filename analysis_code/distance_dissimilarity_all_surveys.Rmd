...
At some point,  I will need to pull in updated data using updated data cleaning procedure. 
Currently  using /Users/zoekitchel/Downloads/FISHGLOB_v1.2_clean.csv not sure if it's right
...
#cleaned by Laura
fishglob_dat <- fread("/Users/zoekitchel/Downloads/FISHGLOB_v1.2_clean.csv")

#limit to 10 years
fishglob_dat[,nyear := length(unique(year)),survey_unit]

View(unique(fishglob_dat[,.(survey_unit, nyear)]))

FishGlob_cleaned.10year <- fishglob_dat[nyear >= 10,]
...
```{r}
all_season_surveys <- unique(FishGlob_cleaned.10year[,survey_unit])
```

Check to see if any are missing CPUE?
```{r missing cpue}
View(FishGlob_cleaned.10year[,sum(is.na(wgt_cpue)),survey_unit])
```
...
length(unique(FishGlob_cleaned.10year[survey == "NEUS",haul_id]))
#this removes 315 hauls from 36792 total hauls (0.9%)
...
#This edit brings 4344957 down to 4332132 observations (Loss of 0.1% of observations)
...
NIGFS 2005 1 GB-NIR 74RY ROT 35 1 ONLY HAS 1 OBSERVATION, SO I'M SKIPPING IT (overall, NIGFS has little coverage, so exclude)


exclude NIGFS
```{r}
FishGlob_cleaned.10year.r <- FishGlob_cleaned.10year.r[survey != "NIGFS",]
all_season_surveys <- unique(FishGlob_cleaned.10year.r[,survey_unit])
```
...
distances_dissimilarities_allyears <- data.table("haul_id1" = integer(),
                                                 ...
                                                 survey = character(),
                                                 season = character(),
                                                 season_survey = character())

for (i in 1:length(all_season_surveys)) {
  
  FishGlob_cleaned.10year_subset <- FishGlob_cleaned.10year.r[survey_unit == all_season_surveys[i],]
  
  #map
  ####unique lat lon
  #lat_lon <- unique(FishGlob_cleaned.10year_subset[,.(latitude, longitude_s)])
    lat_lon <- unique(FishGlob_cleaned.10year_subset[,.(latitude, longitude)])
 ...
 row <- data.table(FishGlob_cleaned.10year_subset[1,survey],
                        ...
                        FishGlob_cleaned.10year_subset[1,as.character(survey_unit)],
  ...
  FishGlob_cleaned.10year_subset <- FishGlob_cleaned.10year_subset[haul_ids_key, on = "haul_id"]
          ...
          if(!(reduced_year[1,survey] %in% abundances_only)) {
            ...
            reduced_year <- reduced_year[complete.cases(reduced_year[,wgt_cpue]),]
              ...
              dissimilarities_abundance_gradient.l[,"abund_biomass" := "abundance"]
              dissimilarities_occurrence_nestedness.l[,"abund_biomass" := "abundance"]
...
all_season_surveys <- unique(distances_dissimilarities_allyears[,survey_unit])
