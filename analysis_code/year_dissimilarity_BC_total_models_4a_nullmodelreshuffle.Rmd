...
for (i in 1:1000){
  ...
  year_run_key <- data.table(year_adj = years_adj, years_adj_null = sample(years_adj, length(years_adj)))
  ...
  if(i == 1){
     all_null_runs_reshuffle_year <- bray_curtis_total_coefs.exp }else{
    all_null_runs_reshuffle_year <- rbind(all_null_runs_reshuffle_year, bray_curtis_total_coefs.exp)
   }
  print(i)
}
...
all_null_runs_reshuffle_year <- all_null_runs_reshuffle_year[true_slopes, on = "survey_unit"]

#take averages and quantiles across all runs
all_null_runs_reshuffle_year[,mean_slope := mean(Year_slope),Survey_Name_Season][,upper_95 := quantile(Year_slope,0.975),Survey_Name_Season][,lower_95 := quantile(Year_slope,0.025),Survey_Name_Season][,true_slope := year_adj]

saveRDS(all_null_runs_reshuffle_year, here::here("output","all_null_runs_reshuffle_year.Rds"))

#plot distribution of slopes for each survey vs. "true" slope
hist_null_slope_year_reshuffle <- ggplot(all_null_runs_reshuffle_year) +
...
ggsave(hist_null_slope_year_reshuffle, filename = "hist_null_slope_year_reshuffle.jpg", path = here::here("figures"), height = 10, width = 12)

#unique values per region
all_null_runs_reshuffle_year.u <- unique(all_null_runs_reshuffle_year[,.(survey_unit, Survey_Name_Season,mean_slope, upper_95, lower_95, true_slope, lwr, upr, Directional_Change, significant, Directional_Change_sig, trend_color)])

#lollipops
errorbar_slope_year_reshuffle <- ggplot() +
  geom_errorbar(data = all_null_runs_reshuffle_year.u, aes(x = reorder(Survey_Name_Season, true_slope) , y = mean_slope, label = Survey_Name_Season, ymin = lower_95, ymax = upper_95), color = "grey50", width = 0) + #add confidence intervals
  geom_point(data = all_null_runs_reshuffle_year.u, aes(x = reorder(Survey_Name_Season, true_slope) , y = true_slope, label = Survey_Name_Season, color = Directional_Change_sig), shape = 18, size = 3) +
  scale_color_manual(values = c("#73BA4D","#E0962C","#cbbfde"), name = "Dissimilarity trend") +
 ...
 # scale_y_continuous(breaks = seq(-0.005, 0.0075, by = 0.0025), labels = c("-0.005","","0", "", "0.005",  "")) +
  ...
  coord_flip() +
  theme_classic() +
  theme(axis.text.y = element_text(face = "bold"), axis.title.y = element_blank(), axis.text.x = element_text(size = 15), axis.title.x = element_text(size = 15), legend.position = c(0.2,0.8), legend.direction = "vertical", legend.text = element_text(size = 15), legend.title = element_text(size = 16))

ggsave(errorbar_slope_year_reshuffle, filename = "errorbar_slope_year_reshuffle.jpg", path = here::here("figures"), height = 5, width = 5)
