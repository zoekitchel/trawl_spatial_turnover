...
```{r}
FishGlob_clean[,tow_abundance_sum := ifelse(survey == "MEDITS", sum(num_cpue),sum(wgt_cpue)),.(haul_id)]
summary(FishGlob_clean$tow_abundance_sum)

tows_by_abundance <- unique(FishGlob_clean[,.(haul_id, tow_abundance_sum)])
tows_noabundance <- unique(FishGlob_clean[tow_abundance_sum == 0,.(haul_id, tow_abundance_sum, survey_unit, year)])

length(unique(FishGlob_clean[tow_abundance_sum == 0,haul_id])) #73 tows out of 216295 have no recorded observations (o for all), 0.03%

FishGlob_clean <- FishGlob_clean[!(haul_id %in% tows_noabundance$haul_id)]
```
...
distances_dissimilarities_allyears <- data.table("bray_curtis_dissimilarity_balanced_mean" = numeric(),
  ...
  FishGlob_clean_subset <- FishGlob_clean_subset[haul_ids_key,  on = "haul_id"]
          ...
          if(!(reduced_year[1, survey] == "MEDITS")) {
            ...
            reduced_year <- reduced_year[complete.cases(reduced_year[, wgt_cpue]), ]
              ...
              dissimilarities_full.r[, "survey" := FishGlob_clean_subset[1, survey]]
              ...
              #bind with full
              distances_dissimilarities_allyears <- rbind(distances_dissimilarities_allyears, dissimilarities_full.r)
