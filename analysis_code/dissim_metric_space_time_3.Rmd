...
Note that this outputs some NA biomass dissimilarity values for SCS and NEUS, and some 0 biomass balues for NZ WCSI and ECSI . I will NOT be using biomass going forward, so it's okay.



##Add in Hill numbers!
Calculate dissimilarity using Hill for both abundance and biomass
```{r loop through all regions and years for hill numbers}

hill_numbers_allyears <- data.table("survey" = character(), 
                                                 "survey_unit" = character(),
                                                "year" = integer(),
                                                "dissimilarity_metric" = character(),
                                                "annual_dissimilarity_value" = numeric())
...
for (i in 1:length(all_survey_units)) {
  ...
  hill_numbers_onereg <- data.table("survey" = character(), 
                                                 "survey_unit" = character(),
                                                "year" = integer(),
                                                "dissimilarity_metric" = character(),
                                                "annual_dissimilarity_value" = numeric())
...
########################################################################
########################################################################
  #Calculate Hill #s based on BIOMASS
          
          if(all_survey_units[i] %in% bio_subset) { #BIOMASS BASED Hill # Calculations
            ...
            reduced_year_bio <- copy(reduced_year.u[complete.cases(reduced_year.u[, wgt_cpue]), ])
              ...
              #Calculate Hill #s
              dissimilarities_abundance_hill_num3 <- hill_taxa_parti_pairwise(communitymatrix, output = "matrix",pairs = c("full"), q = 2)
             
              #make into matrix
              dissimilarities_abundance_hill_num3.m <- dissimilarities_abundance_hill_num3$TD_beta

              #add row and column names as haul ids
              colnames(dissimilarities_abundance_hill_num3.m) <- rownames(dissimilarities_abundance_hill_num3.m) <- key_IDs_subset
            
            
              #reshape dissimilarities
              dissimilarities_abundance_hill_num3.l <- reshape2::melt(dissimilarities_abundance_hill_num3.m, varnames = c("haul_id1","haul_id2"), value.name = "hill_number_biomass_q3")
              
              #and then to data table format
              dissimilarities_abundance_hill_num3.l <- data.table(dissimilarities_abundance_hill_num3.l)
              
              #new column with dissim metric
              dissimilarities_abundance_hill_num3.l[,dissimilarity_metric := "hill_number_biomass_q3"]
              
              #reorder columns
              dissimilarities_abundance_hill_num3.l <- dissimilarities_abundance_hill_num3.l[,.(haul_id1, haul_id2, dissimilarity_metric, hill_number_biomass_q3)]
              
              #change colnames
              colnames(dissimilarities_abundance_hill_num3.l) <- c("haul_id1","haul_id2","dissimilarity_metric","value")
            
              #merge distance with dissimilarity for this year with both metrics of dissimilarity
              dissimilarities_full <- haul_id_distances.l[dissimilarities_abundance_hill_num3.l,  on = c("haul_id1",  "haul_id2")]
              ...
              #bind with full for this reg
              hill_numbers_onereg <- rbind(hill_numbers_onereg, dissimilarities_full.r)
            
          } #closes if it's in bio subset
              
########################################################################
########################################################################
  #Calculate Hill #s based on ABUNDANCE
 ...
 #add column names
              ...
              #reshape dissimilarities
              dissimilarities_abundance_hill_num3.l <- reshape2::melt(dissimilarities_abundance_hill_num3.m,  varnames = c("haul_id1",  "haul_id2"),  value.name = "hill_number_abundance_q3")
             ...
             #new column with dissim metric
              dissimilarities_abundance_hill_num3.l[,dissimilarity_metric := "hill_number_abundance_q3"]
              
              #reorder columns
              dissimilarities_abundance_hill_num3.l <- dissimilarities_abundance_hill_num3.l[,.(haul_id1, haul_id2, dissimilarity_metric, hill_number_abundance_q3)]
  ...
  #save the output from this region
  filename <- paste0(all_survey_units[i],"_hill_number.rds")
  saveRDS(hill_numbers_onereg, here::here("output","hill_numbers","by_region",filename))
  rm(hill_numbers_onereg) #attempt to save some memory here
...
#merge all years into distances_dissimilarities_allyears
files <- list.files(here::here("output","hill_numbers","by_region"))

for(i in 1:length(files)){
  hill_numbers_byreg <- readRDS(here::here("output","hill_numbers","by_region",files[i]))
  hill_numbers_allyears <- rbind(hill_numbers_allyears, hill_numbers_byreg)
}


#Be sure to check which one of these you actually want to un-hash!

saveRDS(hill_numbers_allyears,  here::here("output","hill_numbers", "hill_numbers_allyears.rds"))

hill_numbers_allyears <- readRDS(here::here("output","hill_numbers", "hill_numbers_allyears.rds"))
```


Merge hill #s with dissimilarities
```{r}
...
##Add in Jaccard abundance calculations, and regular Jaccard using VEGAN package while you're at it
