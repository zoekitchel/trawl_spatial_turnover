...
Total tows?
```{r}
length(unique(FishGlob_clean[,haul_id]))
```

Total survey units?
```{r}
length(unique(FishGlob_clean[,survey_unit]))
```

Total species?
```{r}
length(unique(FishGlob_clean[,accepted_name]))
```
...
for (i in 1:length(all_survey_units)) {
  ...
  FishGlob_clean_subset <- FishGlob_clean[survey_unit == all_survey_units[i], ]
...
TO DO: get rid of duplicates, will probably speed things up!!! key_ID!=key_ID2
...
saveRDS(distances_dissimilarities_allyears.r,  here::here("output", "dissimilarities", "distances_dissimilarities_allyears.r.rds"))
```
