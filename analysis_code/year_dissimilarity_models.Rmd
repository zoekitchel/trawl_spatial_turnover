---
title: "Year Dissimilarity Models"
output: html_notebook
---

This code is Script 4 for Kitchel et al. TITLE manuscript.
...
jaccard_total_gam <- 
gam(jaccard_dissimilarity_total_mean ~ year + s(survey_unit,year, bs = "fs", m = 1), #random smooth
                                                         data = distances_dissimilarities_allyears.r)
```
...
bray_curtis_total_gam <- 
gam(bray_curtis_dissimilarity_total_mean ~ year + s(survey_unit, year, bs = "fs", m = 1), #random smooth
                                                         data = distances_dissimilarities_allyears.r)
```
...
model_coefs_reduced_length.unique <- unique(model_coefs_reduced_length[,.(condval,condsd, lwr, upr, survey_unit, year_adj, years_sampled, Directional_Change, hex, Survey_Name_Season, significant)]) 

#extract color hexes
#year adj coef order
color_year_adj_order <- model_coefs_reduced_length.unique[,hex]

#alphabetical order
model_coefs_reduced_length.unique.alpha <- setorder(model_coefs_reduced_length.unique, Survey_Name_Season)
color_alpha_order <- model_coefs_reduced_length.unique.alpha[,hex]

saveRDS(model_coefs_reduced_length.unique, here::here("output","region_stats","model_coefs_reduced_length.unique.Rds"))
```

Bar Plot Coefficient LMER
```{r bar plot of coefficients}
BC_Dissimilarity_Coef_errorbar_reduced <- ggplot() +
...
points_wavylines_bray_year_reduced_gam_nolmer <- ggplot() +
...
points_wavylines_bray_year_reduced_gam_nolmer

#with lmer

points_wavylines_bray_year_reduced_gam <- ggplot() +
  geom_ribbon(data = lmer_bray_balanced_predictions, aes(x = year, ymin = bray_curtis_lmer_preds_lowerCI, ymax = bray_curtis_lmer_preds_upperCI), fill = "grey", alpha = 0.2) +
  geom_point(data = distances_dissimilarities_allyears.r,
             aes(x = year,
                 y = bray_curtis_dissimilarity_balanced_mean,
...
points_wavylines_bray_year_reduced_gam

#just one region that's homogenizing (NZ WCSI)
points_wavylines_bray_year_reduced_gam_NZ_only <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears.r[survey_unit == "NZ-WCSI",],
             aes(x = year,
                 y = bray_curtis_dissimilarity_balanced_mean), alpha = 0.5, size = 1, color = model_coefs_reduced_length.unique[Survey_Name_Season == "New Zealand - West Coast S Island",hex]) +
    geom_line(data = year_survey_unit_expand.dt[survey_unit == "NZ-WCSI",],
             aes(x = year,
                 y = bray_glm_mod_fit),
             color = model_coefs_reduced_length.unique[Survey_Name_Season == "New Zealand - West Coast S Island",hex]) +
  geom_ribbon(data = year_survey_unit_expand.dt[survey_unit == "NZ-WCSI",], aes(x = year, ymin=bray_glm_mod_fit-bray_glm_mod_fit_SE, ymax=bray_glm_mod_fit+bray_glm_mod_fit_SE), alpha=0.3, fill = model_coefs_reduced_length.unique[Survey_Name_Season == "New Zealand - West Coast S Island",hex]) + #add standard error
 # geom_line(data = lmer_bray_balanced_predictions, aes(x = year, y = bray_curtis_lmer_preds), color = "black") +
...
points_wavylines_bray_year_reduced_gam_NZ_only

#just two regions, another that's differentiating (SEUS Summer)

points_wavylines_bray_year_reduced_gam_NZ_SEUS_Summer_only <- ggplot() +
 ...
 # geom_line(data = lmer_bray_balanced_predictions, aes(x = year, y = bray_curtis_lmer_preds), color = "black") +
    scale_color_manual(name = "Survey Unit", values = c(model_coefs_reduced_length.unique[Survey_Name_Season %in% c("New Zealand - West Coast S Island","Southeast US Summer"),hex])) +
  scale_fill_manual(guide = "none", values = c(model_coefs_reduced_length.unique[Survey_Name_Season %in% c("New Zealand - West Coast S Island","Southeast US Summer"),hex])) +
    lims(x = c(min(distances_dissimilarities_allyears.r[,year]),max(distances_dissimilarities_allyears.r[,year])),
       y = c(0.15,0.9)) +
  theme_classic() +
  xlab("Year") +
...
points_wavylines_bray_year_reduced_gam_NZ_SEUS_Summer_only

#just three regions, another one that's stable, (Canada Scotian Shelf)

points_wavylines_bray_year_reduced_gam_NZ_SEUS_ScoShelf_only <- ggplot() +
 ...
 # geom_line(data = lmer_bray_balanced_predictions, aes(x = year, y = bray_curtis_lmer_preds), color = "black") +
    scale_color_manual(name = "Survey Unit", values = c(model_coefs_reduced_length.unique[Survey_Name_Season %in% c("New Zealand - West Coast S Island","Southeast US Summer","Canada - Scotian Shelf"),hex])) +
  scale_fill_manual(guide = "none", values = c(model_coefs_reduced_length.unique[Survey_Name_Season %in% c("New Zealand - West Coast S Island","Southeast US Summer","Canada - Scotian Shelf"),hex])) +
    lims(x = c(min(distances_dissimilarities_allyears.r[,year]),max(distances_dissimilarities_allyears.r[,year])),
       y = c(0.15,0.9)) +
  theme_classic() +
  xlab("Year") +
...
points_wavylines_bray_year_reduced_gam_NZ_SEUS_ScoShelf_only

ggsave(points_wavylines_bray_year_reduced_gam_NZ_only, path = here::here("figures"), filename ="points_wavylines_bray_year_reduced_gam_NZ_only.jpg", height = 5, width = 5, unit = "in")
ggsave(points_wavylines_bray_year_reduced_gam_NZ_SEUS_Summer_only, path = here::here("figures"), filename ="points_wavylines_bray_year_reduced_gam_NZ_SEUS_Summer_only.jpg", height = 5, width = 5, unit = "in")
ggsave(points_wavylines_bray_year_reduced_gam_NZ_SEUS_ScoShelf_only, path = here::here("figures"), filename ="points_wavylines_bray_year_reduced_gam_NZ_SEUS_ScoShelf_only.jpg", height = 5, width = 5, unit = "in")
points_wavylines_bray_year_reduced_gam
ggsave(points_wavylines_bray_year_reduced_gam_nolmer, path = here::here("figures"), filename ="points_wavylines_bray_year_reduced_gam_nolmer.jpg", height = 5, width = 5, unit = "in")

ggsave(points_wavylines_bray_year_reduced_gam, path = here::here("figures"), filename ="points_wavylines_bray_year_reduced_gam.jpg", height = 5, width = 5, unit = "in")
...
BC_GAM_LMER_merge <- plot_grid(points_wavylines_bray_year_reduced_gam + theme(legend.position = "none", plot.margin = margin(2,0,0,0, unit = "cm")), BC_Dissimilarity_Coef_errorbar_reduced + theme(legend.position = "none", plot.margin = margin(2,0,0,0, unit = "cm")), labels = c("","Survey Unit  ",""), vjust = 1.5, hjust = 0)

BC_GAM_LMER_merge_legend <- plot_grid(BC_GAM_LMER_merge, get_legend(BC_Dissimilarity_Coef_errorbar_reduced), rel_widths = c(10,2))

ggsave(BC_GAM_LMER_merge_legend, path = here::here("figures"), filename = "BC_GAM_LMER_merge_legend.png", height = 7, width = 14, units = "in")
```
```
