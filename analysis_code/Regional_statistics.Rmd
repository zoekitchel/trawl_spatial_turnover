...
spp_num_vs_coef <- ggplot() +
  ...
  geom_smooth(data = region_stats, aes(x = spp_num, y = bray_coef), method = "lm", color = "black", se = F) +
...
study_duration_vs_coef <- ggplot() +
    geom_errorbar(data = region_stats, aes(x = study_duration, y = bray_coef, ymin = bray_coef_lwr, ymax = bray_coef_upr, color = `Significant Trends`), alpha = 0.8, linewidth = 0.3) + #add confidence intervals
        geom_point(data = region_stats, aes(x = study_duration , y = bray_coef, color = `Significant Trends`), stat = 'identity', size = 1) +
    scale_color_manual(values = c("#91c874", "#e7ac5b", "#cbbfde"), name = "Directional Change") +
  geom_hline(yintercept = 0, size = 0.5) +
  ylab("Dissimilarity coefficient") +
...
ggsave(study_duration_vs_coef, path = here::here("figures","bray_coef_vs_survey_characteristics"), filename = "study_duration_vs_coef.jpg", height =3, width = 5, unit = "in")

#linear model
summary(lm(abs(bray_coef)~study_duration, data = region_stats)) #not significant
summary(lm(bray_coef~study_duration, data = region_stats)) #not significant
...
mid_lat_vs_coef <- ggplot() +
    ...
    scale_color_manual(values = c("#91c874", "#e7ac5b", "#cbbfde"), name = "Directional Change") +
  geom_hline(yintercept = 0, size = 0.5) +
  geom_smooth(data = region_stats, aes(x = mid_lat, y = bray_coef), method = "lm", color = "black", se = F) +
  ylab("Dissimilarity coefficient") +
...
mean_temp_vs_coef <- ggplot() +
    ...
    scale_color_manual(values = c("#91c874", "#e7ac5b", "#cbbfde"), name = "Directional Change") +
  geom_hline(yintercept = 0, size = 0.5) +
  geom_smooth(data = region_stats, aes(x = mean_temp, y = bray_coef), method = "lm", color = "black", se = F) +
  ylab("Dissimilarity coefficient") +
