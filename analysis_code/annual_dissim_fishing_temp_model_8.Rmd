...
For temperature, we will compare performance of:
...
model_avg_notemp <- subset(model_set, !("summed_tonnes_scaled_byreg" %in% attr(terms(model.set), "term.labels")))
  
#Calculate marginal R^2 values

# 5. Filter model set to exclude the variable of interest
model_set_minus_fishing <- subset(dd, !grepl("summed_tonnes_scaled_byreg", formula))

# Get all models from dredge output
all_models <- get.models(dd, subset = TRUE)

# Filter out ones that contain the term
models_minus_fishing <- Filter(function(m) {
  !any(grepl("summed_tonnes_scaled_byreg", attr(terms(m), "term.labels")))
}, all_models)

model_selection_minus_fishing <- model.sel(models_minus_fishing)

#Calculate R^2 no fishing
nofishing_summary <- summary(get.models(model_selection_minus_fishing, subset = 1)[[1]]) #0.93
r2_nofishing <- signif(nofishing_summary$r.squared)
r2_nofishing 

# Filter out ones that contain the term
models_minus_temperature <- Filter(function(m) {
  !any(grepl("yearly_min_bypoint_avg.scaledacrossall", attr(terms(m), "term.labels")))
}, all_models)

model_selection_minus_temperature <- model.sel(models_minus_temperature)

#Calculate R^2 no temperature
notemperature_summary <- summary(get.models(model_selection_minus_temperature, subset = 1)[[1]]) #0.94
r2_temperature <- signif(notemperature_summary$r.squared)
r2_temperature

#Exclude both
models_minus_temp_and_fishing <- Filter(function(m) {
  terms <- attr(terms(m), "term.labels")
  # Return TRUE if neither fishing nor temp (or interactions involving them) are in terms
  !any(grepl("summed_tonnes_scaled_byreg", terms)) &&
    !any(grepl("yearly_min_bypoint_avg.scaledacrossall", terms))
}, all_models)


model_selection_minus_temp_and_fishing <- model.sel(models_minus_temp_and_fishing)

#Calculate R^2 no fishing
notemp_nofishing_summary <- summary(get.models(model_selection_minus_temp_and_fishing, subset = 1)[[1]]) #0.93
r2_temp_and_fishing_exclude <- signif(notemp_nofishing_summary$r.squared)
...
#If we ignore two coldest years, we have roughly points at (2.07,0.76) and (2.2, 0.69), slope = -0.5, which matches what we get (also note large confidence bounds)

#So, the question is, what is leading the model to ignore these two years? What explains this variation in the coldest years?

#Just run model for AI
summary(lm(data = dissimilarity_covariates_dredge.dt[survey_unit == "AI"], annual_dissimilarity_value ~ yearly_min_bypoint_avg))
...
```
What leads to variance being explained by variables OTHER than temperature in coldest years
So, in summary
- Very large sampling area and very small sampling areas are both linked to years of low dissimilarity
- *Very large sampling area* coincides with lowest temperatures
- Annual dissimilarity value increases with richness
- *Lowest temperature observation coincides with lowest richness*
- Low number of hauls led to very low annual dissimilarity values
...
For the Aleutian islands, the coldest years were also:
- the years of largest sampling area
- the years of lowest richness
- the years of lowest sampling effort (# tows)
- the years of deepest sampling
