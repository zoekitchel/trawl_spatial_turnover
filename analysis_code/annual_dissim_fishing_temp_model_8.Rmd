...
#covariance matrix
cov_m <- vcov(model_avg_delta4)

#extract coefficients
interaction_avg_model_coef <- model_avg_values[c(7,39:101),.(coef_name, coef, se)]

#interaction standard error = sqrt(var(b1) + var(b2) + 2cov(b1~b2))
                              #sqrt(var(summed_tonnes SE^2) + var(Aleutian Islands SE^2) + 2cov(summed_tonnes ~ intercept (-8.131722e-03)))
...
#Other summed_tonnes_scaled_byreg (edit both coefficient and )
interaction_avg_model_coef[3:33,survey_unit := substr(coef_name, 39, str_length(coef_name))]
interaction_avg_model_coef[3:33,coef_true := interaction_avg_model_coef[1,coef_true]+coef]
interaction_avg_model_coef[3:33,predictor := "Relative fishing catch"]
