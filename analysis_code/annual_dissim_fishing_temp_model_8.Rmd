...
(tow_density_survey_facet_1_20 <- ggplot(data = tows_year[survey_unit %in% all_surveys[1:20]]) +
  labs(x = "Year",  y = "Tow density (tows per km^2)") +
...
ggsave(tow_density_dissimilarity_1_20, path = here::here("figures"), filename = "tow_density_dissimilarity_1_20.jpg", height = 7, width =9)
...
tow_density_dissimilarity_21_34 <- 
  ...
  scale_color_viridis() +
  facet_wrap(~Survey_Name_Season, ncol = 4) +
  labs(x = "Tow density (tows per km^2)", y = "β-diversity") +
    xlim(c(min_haul_density-0.0001, max_haul_density+0.0001)) +
    ylim(c(min_dissimilarity-0.01,max_dissimilarity+0.01)) +
  theme_classic()

ggsave(tow_density_dissimilarity_21_34, path = here::here("figures"), filename = "tow_density_dissimilarity_21_34.jpg", height = 7, width =9)
```
...
global_mod_sbt_table <- global_mod_temp_table[,.(Rank,`Model structure`, deltaAICc)]
...
Dissimilarity decreases with increasing Julian day of sampling
