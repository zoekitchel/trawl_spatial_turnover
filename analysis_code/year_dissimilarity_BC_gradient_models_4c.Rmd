---
title: "Year Biomass Gradient Dissimilarity Models (nestedness)"
output: html_notebook
---

This code is Script 4c for Kitchel et al. TITLE manuscript.
...
points_wavylines_bray_gradient_year_reduced_gam_EBS_only <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears.r[survey_unit == "EBS",],
             aes(x = year,
                 y = bray_curtis_dissimilarity_gradient_mean), alpha = 0.5, size = 1
            # , 
            # color = BC_gradient_model_coefs_reduced_length.unique[Survey_Name_Season == "Eastern Bering Sea",hex]
             ) +
  #  geom_line(data = year_survey_unit_expand.dt[survey_unit == "EBS",],
   #          aes(x = year,
    #             y = bray_glm_mod_fit),
     #        color = BC_gradient_model_coefs_reduced_length.unique[Survey_Name_Season == "Eastern Bering Sea",hex]) +
 # geom_ribbon(data = year_survey_unit_expand.dt[survey_unit == "EBS",], aes(x = year, ymin=bray_glm_mod_fit-bray_glm_mod_fit_SE, ymax=bray_glm_mod_fit+bray_glm_mod_fit_SE), alpha=0.3, fill = BC_gradient_model_coefs_reduced_length.unique[Survey_Name_Season == "Eastern Bering Sea",hex]) + #add standard error
 # geom_line(data = lmer_bray_gradient_predictions, aes(x = year, y = bray_curtis_lmer_preds), color = "black") +
...
points_wavylines_bray_gradient_year_reduced_gam_EBS_only
...
ggsave(points_wavylines_bray_gradient_year_reduced_gam_EBS_only, path = here::here("figures"), filename ="points_wavylines_bray_gradient_year_reduced_gam_EBS_only.jpg", height = 5, width = 9, unit = "in")
