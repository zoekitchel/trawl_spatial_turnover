...
unique_year_survey_spp[,years_sampled_survey := uniqueN(year),survey_unit][,years_spp_occurs := .N,.(accepted_name, survey_unit)][,perc_occurence := round(years_spp_occurs/years_sampled_survey,2)][,keep_drop := ifelse(perc_occurence>=0.3333,T,F)] #keep_drop = TRUE if above 0.33 presence, FALSE if below 0.33 presence
...
FishGlob_clean_twothirdsthird_included <- FishGlob_clean_onethird[keep_drop == T,]

FishGlob_clean_onethird_excluded <- FishGlob_clean_onethird[keep_drop == F,]

#drops 
nrow(FishGlob_clean_onethird_excluded)
#14511/2733591 rows (0.5% of observations)
```
...
#spp included for 85%
survey_unit_spp_list_included_85perc <- list()
for (i in 1:length(all_survey_units)) {
  survey_unit_spp_list_included_85perc[i] <- list(sort(unique(FishGlob_clean_15perc_excluded[survey_unit == all_survey_units[i],]$accepted_name)))
}

names(survey_unit_spp_list_included_85perc) <- paste0(all_survey_units,", include 85% of species with highest biomass")

#spp included for 2/3 occurence

survey_unit_spp_list_included_twothirds <- list()
for (i in 1:length(all_survey_units)) {
  full_spp <- unique(FishGlob_clean[survey_unit == all_survey_units[i],]$accepted_name)
  survey_unit_spp_list_included_twothirds[i] <- list(sort(unique(FishGlob_clean_twothirdsthird_included[survey_unit == all_survey_units[i],]$accepted_name)))
}

names(survey_unit_spp_list_included_twothirds) <- paste0(all_survey_units,", include because present for more than 2/3 of years")

survey_unit_spp_list_all <- c(survey_unit_spp_list,survey_unit_spp_list_included_85perc , survey_unit_spp_list_included_twothirds)
...
```{r}
FishGlob_clean_twothirdsthird_included[,tow_abundance_sum := ifelse(survey == "MEDITS", sum(num_cpue),sum(wgt_cpue)),.(haul_id)]
summary(FishGlob_clean_twothirdsthird_included$tow_abundance_sum)

tows_by_abundance <- unique(FishGlob_clean_twothirdsthird_included[,.(haul_id, tow_abundance_sum)])
tows_noabundance <- unique(FishGlob_clean_twothirdsthird_included[tow_abundance_sum == 0,.(haul_id, tow_abundance_sum, survey_unit, year)])

length(unique(FishGlob_clean_twothirdsthird_included[tow_abundance_sum == 0,haul_id])) 

FishGlob_clean_twothirdsthird_included <- FishGlob_clean_twothirdsthird_included[!(haul_id %in% tows_noabundance$haul_id)]
```
...
for (i in 1:length(all_survey_units)) {
  ...
  FishGlob_clean_twothirdsthird_included_subset <- FishGlob_clean_twothirdsthird_included[survey_unit == all_survey_units[i], ]
  
  #map
  ####unique lat lon
  #add column with adjusted longitude for few surveys that cross dateline (NZ-CHAT and AI)
  FishGlob_clean_twothirdsthird_included_subset[,longitude_adj := ifelse((survey_unit %in% c("AI","NZ-CHAT") & longitude > 0),longitude-360,longitude)]

    lat_lon <- unique(FishGlob_clean_twothirdsthird_included_subset[, .(latitude,  longitude_adj)])

   #list years
  FishGlob_clean_twothirdsthird_included_subset[, year:= as.numeric(year)] #make numeric
  setorder(FishGlob_clean_twothirdsthird_included_subset,  year)
  years <- unique(FishGlob_clean_twothirdsthird_included_subset[, year])
  
  #haul id keys
  haul_ids <- unique(FishGlob_clean_twothirdsthird_included_subset[, haul_id])
  haul_ids_key <- data.table(haul_id = haul_ids,  key_ID = seq(1, length(haul_ids),  by = 1))
  
  
  #convert haul_ids to numeric key_ids
  FishGlob_clean_twothirdsthird_included_subset <- FishGlob_clean_twothirdsthird_included_subset[haul_ids_key,  on = "haul_id"]

          for (j in 1:length(years)) {
            reduced_year <- FishGlob_clean_twothirdsthird_included_subset[year == years[j], ]
          ...
          if(!(reduced_year[1, survey] == "MEDITS")) {
            ...
            reduced_year <- reduced_year[complete.cases(reduced_year[, wgt_cpue]), ]
              ...
              dissimilarities_full.r[, "survey" := FishGlob_clean_twothirdsthird_included_subset[1, survey]]
