...
region_stats.format[,`Mean annual CPUE (kg/km^2)`:=mean_annual_cpue][,`Mean annual tow count` := round(mean_haul_id_count_annual)][,`Latitude range (˚)`:= round(lat_range,2)][,`Longitude range (˚)`:=round(lon_range,2)][,`Area (1000s of kms)`:=round(area_km/1000,0)][,`Mid-depth (m)`:= round(mid_depth,2)][,`Depth range (m)`:= round(depth_range,2)][,`Mid-latitude (˚N)`:= round(mid_lat,2)][,`Mean bottom temperature (˚C)`:= round(mean_temp,2)][,`Mean annual fisheries landings (1000s of tonnes)`:= round(mean_landings/1000,0)][,`Number of species across all years`:= spp_num][,`Survey length (years)`:=study_period][,`Total years sampled`:=years_sampled][,`First year`:=first_year][,`Last year`:=last_year]
...
```{r prop test}
                ...
                #N, S
homogenizers <- c(7,0)
differentiators <- c(8,6)
total_surveys <- c(33,9)


prop.test(homogenizers,total_surveys) #p value >> 0.05, so groups are not differennt
prop.test(differentiators,total_surveys) #p value < 0.05, so groups are significantly different
