...
years <- sort(unique(distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total",]$year))


for (i in 1:1000){

  jaccard_dissimilarity_time <- distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total",.(survey_unit, year, year_adj, annual_dissimilarity_value)]
  ...
  jaccard_total_coefs.r[,survey_unit := substr(var, 23, str_length(var))][var == "years_null",survey_unit := "AI"]
...
(reshuffle_slope_comparison_plot <- ggplot() +
  geom_histogram(data = jaccard_total_coefs.allruns, aes(survey_unit_coefficient), fill = "#595959", binwidth = 0.001) +
  labs(x = "β diversity trend (null)", y = "Count (in 10,000s)") +
    scale_y_continuous(breaks = c(0,10000,20000,30000), labels = c("0","1","2","3")) +
    geom_vline(xintercept = median(jaccard_total_coefs.allruns$survey_unit_coefficient)) +
   geom_vline(xintercept = quantile(jaccard_total_coefs.allruns$survey_unit_coefficient, c(0.025,0.975)), linetype = "dashed") +
    xlim(min(c(jaccard_total_coefs.allruns$survey_unit_coefficient,jaccard_total_coefs.r_true$survey_unit_coefficient))-0.0035,
         max(c(jaccard_total_coefs.allruns$survey_unit_coefficient,jaccard_total_coefs.r_true$survey_unit_coefficient))+0.003) +
  theme_classic())

(true_slope_comparison_plot <- ggplot() +
  geom_histogram(data = jaccard_total_coefs.r_true, aes(survey_unit_coefficient), fill = "#595959", binwidth = 0.001) +
  labs(x = "β diversity trend (observed)", y = "Count") +
    geom_vline(xintercept = median(jaccard_total_coefs.allruns$survey_unit_coefficient)) +
    geom_vline(xintercept = quantile(jaccard_total_coefs.allruns$survey_unit_coefficient, c(0.025,0.975)), linetype = "dashed") +
    xlim(min(c(jaccard_total_coefs.allruns$survey_unit_coefficient,jaccard_total_coefs.r_true$survey_unit_coefficient))-0.003,
         max(c(jaccard_total_coefs.allruns$survey_unit_coefficient,jaccard_total_coefs.r_true$survey_unit_coefficient))+0.003) +
  theme_classic())
...
years_adj <- sort(unique(distances_dissimilarities_allyears[dissimilarity_metric == "jaccard_dissimilarity_total",]$year_adj))
...
#for (i in 1:1000)){ #if we want 1000 runs
 ...
 # start_year <- sample(1:length(years_adj), 1) #if instead we want 1000 random runs
  ...
  jaccard_dissimilarity_time  <- jaccard_dissimilarity_time[year_run_key, on = "year_adj"]
  
  #run overall model
  jaccard_total_lmer <- lmer(annual_dissimilarity_value ~ years_adj_null + (1 + years_adj_null|survey_unit),data = jaccard_dissimilarity_time)
  
    
  jaccard_total_coefs <- data.table(coef(jaccard_total_lmer)[[1]])
  jaccard_total_coefs[,survey_unit := rownames(coef(jaccard_total_lmer)[[1]])][,Year_slope := signif(years_adj_null,3)][,Intercept := signif(`(Intercept)`,3)]
  jaccard_total_coefs <- jaccard_total_coefs[color_link, on = "survey_unit"]
  
  jaccard_total_coefs[,run := i]
  
  jaccard_total_coefs.exp <- jaccard_total_coefs[,.(run, Survey_Name_Season, survey_unit, Intercept, Year_slope)]
  
  if(i == 1){
     all_null_runs_cyclic_shift <- jaccard_total_coefs.exp }else{
    all_null_runs_cyclic_shift <- rbind(all_null_runs_cyclic_shift, jaccard_total_coefs.exp)
   }
  print(i)
}
...
true_slopes <- jaccard_total_model_coefs_reduced_length.unique[,.(lwr,upr, survey_unit, year_adj, Directional_Change, significant, Directional_Change_sig, trend_color)]
...
errorbar_slope_year_cyclic_shift <- ggplot() +
  ...
  scale_color_manual(values = c("#FFC109","#D81C60","#1E88E5"), name = "β diversity trend\n(Jaccard dissimilarity)", guide="none") +
