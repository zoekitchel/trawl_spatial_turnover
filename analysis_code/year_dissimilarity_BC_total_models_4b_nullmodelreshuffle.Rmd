...
bray_curtis_total_coefs.r <- read_csv(here::here("output","bray_curtis_total_coefs.r.csv"))
...
#95% quantile of poisson distributions (both count homogenization and count differentiation)
# Example with a Poisson distribution
lambda_homogenization <- mean(all_null_runs_reshuffle_year[,homogenizing])
lambda_differentiation <- mean(all_null_runs_reshuffle_year[,differentiating])

# Calculate the 95th percentile for the Poisson distribution
percentile_95_poisson_homogenization <- qpois(0.95, lambda_homogenization)
percentile_95_poisson_differentiation <- qpois(0.95, lambda_differentiation)
...
(reshuffle_differentiation_plot <- ggplot() +
  geom_histogram(data = all_null_runs_reshuffle_year, aes(differentiating), binwidth = 1) +
      geom_vline(xintercept = sum(bray_curtis_total_coefs.r_true$differentiating), color = "red") +
          geom_vline(xintercept = percentile_95_poisson_differentiation, linetype = "dashed") +
...
(reshuffle_homogenizing_plot <- ggplot() +
  geom_histogram(data = all_null_runs_reshuffle_year, aes(homogenizing), binwidth = 1) +
      geom_vline(xintercept = sum(bray_curtis_total_coefs.r_true$homogenizing), color = "red") +
          geom_vline(xintercept = percentile_95_poisson_homogenization, linetype = "dashed") +
