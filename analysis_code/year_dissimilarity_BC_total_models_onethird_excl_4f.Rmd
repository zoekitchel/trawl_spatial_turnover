...
Script 4f for Kitchel et al. 2024 in prep taxonomic diversity manuscript.
...
jaccard_total_gam <- gam(annual_dissimilarity_value ~ year + s(survey_unit, year, bs = "fs", m = 1),#random smooth
                            data = distances_dissimilarities_allyears_onethirdyears_excluded[dissimilarity_metric == "jaccard_dissimilarity_total"])
...
jaccard_total_lme <- lme(annual_dissimilarity_value ~ year_adj, random = (~1 + year_adj|survey_unit),data = distances_dissimilarities_allyears_onethirdyears_excluded[dissimilarity_metric == "jaccard_dissimilarity_total"])

#Jaccard total with lmer
jaccard_total_lmer <- lmer(annual_dissimilarity_value ~ year_adj + (1 + year_adj|survey_unit),data = distances_dissimilarities_allyears_onethirdyears_excluded[dissimilarity_metric == "jaccard_dissimilarity_total"]) #FYI doesn't converge
...
jaccard_total_lm <- lm(annual_dissimilarity_value ~ year * survey_unit,data = distances_dissimilarities_allyears_onethirdyears_excluded[dissimilarity_metric == "jaccard_dissimilarity_total"])
...
jaccard_total_coefs <- data.table(summary(jaccard_total_lm)$coefficients)
  ...
  jaccard_total_coefs.r_onethirdyears_excluded <- jaccard_total_coefs[c(2,36:68),]
  
  #adjust survey unit name by deleting beginning of string
  jaccard_total_coefs.r_onethirdyears_excluded[,survey_unit := substr(var, 17, str_length(var))][var == "year",survey_unit := "AI"]
  
  #calculate interaction coefficients
  AI_estimate <- jaccard_total_coefs.r_onethirdyears_excluded[1,Estimate]
  jaccard_total_coefs.r_onethirdyears_excluded[1,survey_unit_coefficient := AI_estimate]
  jaccard_total_coefs.r_onethirdyears_excluded[2:34,survey_unit_coefficient := (AI_estimate + Estimate)]
  
  #homogenizing vs differentiating
  jaccard_total_coefs.r_onethirdyears_excluded[,differentiating := ifelse((`Std. Error`< abs(survey_unit_coefficient) & survey_unit_coefficient > 0),1,0)][,homogenizing := ifelse((`Std. Error`< abs(survey_unit_coefficient) & survey_unit_coefficient < 0),1,0)]
  
  #lower and upper CI
    jaccard_total_coefs.r_onethirdyears_excluded[,lwr := survey_unit_coefficient-`Std. Error`][,upr:= survey_unit_coefficient+`Std. Error`]
...
(jaccard_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_onethirdyears_excluded <- ggplot(data = jaccard_total_coefs.r_onethirdyears_excluded, aes(x = reorder(Survey_Name_Season, survey_unit_coefficient) , y = survey_unit_coefficient), label = Survey_Name_Season) +
...
#full dataset coefs
jaccard_total_coefs.r[,jaccard_coefficient := survey_unit_coefficient]
...
jaccard_total_coefs.r_15perc_excluded[,jaccard_coefficient_15percexclude := survey_unit_coefficient]
...
jaccard_total_coefs.r_onethirdyears_excluded[,onethirdyears_relative_coefficient := survey_unit_coefficient][,Directional_Change_onethirdyears := Directional_Change_sig][,trend_color_jaccard_onethirdyears_excluded := trend_color_jaccard]
...
distances_dissimilarities_allyears_onethirdyears_excluded_jaccard <- distances_dissimilarities_allyears_onethirdyears_excluded[color_link_trend_jaccard, on = "survey_unit"]
distances_dissimilarities_allyears_onethirdyears_excluded_jaccard <- distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[dissimilarity_metric == "jaccard_dissimilarity_total_abundance"]
#and again for jaccard
distances_dissimilarities_allyears_onethirdyears_excluded_jaccard <- distances_dissimilarities_allyears_onethirdyears_excluded[color_link_trend_jaccard, on = "survey_unit"]
distances_dissimilarities_allyears_onethirdyears_excluded_jaccard<- distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[dissimilarity_metric == "jaccard_dissimilarity_total"]

#change survey_unit to factor
distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[,survey_unit := factor(survey_unit)]
year_survey_unit_expand_jaccard.dt[,survey_unit := factor(survey_unit)]

#check that survey_unit in alpha order
sort(unique(distances_dissimilarities_allyears_onethirdyears_excluded_jaccard$survey_unit)) == levels(distances_dissimilarities_allyears_onethirdyears_excluded_jaccard$survey_unit)
...
trend_colors_jaccard.dt <- unique(distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[dissimilarity_metric == "jaccard_dissimilarity_total",.(survey_unit, Survey_Name_Season, trend_color_jaccard)])
...
points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded <- ggplot() +
  ...
  #points by trend
  geom_point(data = distances_dissimilarities_allyears_onethirdyears_excluded[dissimilarity_metric =="jaccard_dissimilarity_total"], cols = "year_adj",
             aes(x = year,
  ...
  theme_classic() +
  lims(x = c(min(distances_dissimilarities_allyears_onethirdyears_excluded[dissimilarity_metric == "jaccard_dissimilarity_total",year]),max(distances_dissimilarities_allyears_onethirdyears_excluded[dissimilarity_metric == "jaccard_dissimilarity_total",year]))
       ) +
  xlab("Year\n") +
ylab("β-diversity\n(Jaccard dissimilarity)") +
  theme(legend.position = "null", axis.text = element_text(size = 15), axis.title = element_text(size = 15))

points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded

ggsave(points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded, path = here::here("figures"), filename ="points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded.jpg", height = 6, width = 11, unit = "in")

#plot for legend
directional_change_legend_plot_colorbytrend <- jaccard_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_onethirdyears_excluded + 
...
######################################################
##################JACCARDTOTAL
jaccard_total_GAM_LM_merge_legend_colorbytrend_onethirdyears_excluded <- ggdraw(xlim = c(0, 40.5), ylim = c(0, 21)) +
    draw_plot(points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded + theme(axis.text = element_text(size = 14)),
                                         x = 1, y = 1, width = 20, height = 20) +
    draw_plot(jaccard_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_LINEAR_MODEL_onethirdyears_excluded +
...
ggsave(jaccard_total_GAM_LM_merge_legend_colorbytrend_onethirdyears_excluded, path = here::here("figures"), filename = "jaccard_total_GAM_LM_merge_legend_colorbytrend_onethirdyears_excluded.png", height = 8, width = 14, units = "in")
```

#THIS PORTION DOES NOT RUN BELOW BECAUSE CHILE DOES NOT HAVE ENOUGH DATA AFTER THIS SENSITIVITY ANALYSIS
...
#list of surveys
survey_unit.list_jaccard <- levels(factor(distances_dissimilarities_allyears_onethirdyears_excluded_jaccard$survey_unit))
...
for (i in 1:length(survey_unit.list_jaccard)) {
  GAM_LM_model_comparisons_jaccard_total_single <- compare_models(distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[survey_unit == survey_unit.list_jaccard[i],])
...
GAM_LM_model_comparisons_jaccard_total.f_onethirdyears_excluded <- GAM_LM_model_comparisons_jaccard_total[,-2]

colnames(GAM_LM_model_comparisons_jaccard_total.f_onethirdyears_excluded) <- c("Survey","Model comparison", "GAM AICc","GAM p-value","LM AICc", "LM p-value")

fwrite(GAM_LM_model_comparisons_jaccard_total.f_onethirdyears_excluded, here::here("output","GAM_LM_model_comparisons_jaccard_total.f_onethirdyears_excluded.csv"))
...
for (i in 1:length(survey_unit.list_jaccard)) {
  st_dev_pred_dissim <- GAM_outputs(distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[survey_unit == survey_unit.list_jaccard[i]])
...
GAM_lm_SD_jaccard_total.f_onethirdyears_excluded <- GAM_lm_SD_jaccard_total[,.(Survey, SD_pred_dissim, aicc_gam, gam_p, aicc_lm, lm_p, comparison)]

colnames(GAM_lm_SD_jaccard_total.f_onethirdyears_excluded) <- c("Survey","SD over time of predicted GAM values", "GAM AICc","GAM p-value","LM AICc", "LM p-value","Model comparison")

View(GAM_lm_SD_jaccard_total.f_onethirdyears_excluded)

fwrite(GAM_lm_SD_jaccard_total.f_onethirdyears_excluded, here::here("output","GAM_lm_SD_jaccard_total.f_onethirdyears_excluded.csv"))
...
distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[,Survey_Name_Season := factor(Survey_Name_Season, levels = trend_colors_jaccard.dt$Survey_Name_Season, ordered = T)]

#change year to integer
distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[,year:=as.integer(year)]

#split into 2 and use facet
#first 17
points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded_individual_facet_1_20 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[survey_unit  %in% all_survey_names[c(1:20)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value), alpha = 0.3) +

    geom_smooth(data = distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[survey_unit  %in% all_survey_names[c(1:20)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value,
                 color = survey_unit,
...
ggsave(points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded_individual_facet_1_20, path =  here::here("figures"), filename = "points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded_individual_facet_1_20.png", height = 12, width = 9.5)

#second portion
points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded_individual_facet_21_34 <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[survey_unit  %in% all_survey_names[c(21:34)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value), alpha = 0.3) +

    geom_smooth(data = distances_dissimilarities_allyears_onethirdyears_excluded_jaccard[survey_unit  %in% all_survey_names[c(21:34)]],
             aes(x = as.integer(year),
                 y = annual_dissimilarity_value,
                 color = survey_unit,
                 fill = survey_unit), method = "lm", linewidth = 0.5) +
...
ggsave(points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded_individual_facet_21_34, path =  here::here("figures"), filename = "points_wavylines_jaccard_total_year_reduced_gam_colorbytrend_onethirdyears_excluded_individual_facet_21_34.png", height = 10, width = 9.5)
