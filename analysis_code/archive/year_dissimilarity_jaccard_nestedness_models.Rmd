---
title: "Year Nestedness Dissimilarity Models (nestedness component)"
output: html_notebook
---

This code is Script 4 v4 for Kitchel et al. TITLE manuscript.

(We expect this to be in the supplement)
...
Jaccard
```{r Jaccard gams}
jaccard_nestedness_gam <- gam(jaccard_dissimilarity_nestedness_mean ~ year + s(survey_unit, year, bs = "fs", m = 1),#random smooth
...
*These all converged*
```{r jaccard}
jaccard_nestedness_lmer <- lmer(jaccard_dissimilarity_nestedness_mean ~ year_adj + (1 + year_adj|survey_unit),data = distances_dissimilarities_allyears.r)

summary(jaccard_nestedness_lmer)
...
lmer_jaccard_nestedness_predictions <- data.table(year = lmer_year, year_adj = lmer_year_adj)

#confidence intervals
jaccard_nestedness_lmer_confint <- confint(jaccard_nestedness_lmer)

#populate data table of lmer predictions
lmer_jaccard_nestedness_predictions[,jaccard_lmer_preds := fixef(jaccard_nestedness_lmer)[[1]] + fixef(jaccard_nestedness_lmer)[[2]] * year_adj][,jaccard_lmer_preds_lowerCI := jaccard_nestedness_lmer_confint[5] + jaccard_nestedness_lmer_confint[6] * year_adj][,jaccard_lmer_preds_upperCI := jaccard_nestedness_lmer_confint[11] + jaccard_nestedness_lmer_confint[12] * year_adj]
```
...
###Move forward with Jaccard Nestedness (likelyto supplement)
...
model_coefs_reduced <- data.table(transform(as.data.frame(ranef(jaccard_nestedness_lmer)), lwr = condval - 1.96*condsd, upr = condval + 1.96*condsd))
...
min_jaccard_reduced <- min(distances_dissimilarities_allyears.r[,jaccard_dissimilarity_nestedness_mean], na.rm = T)
max_jaccard_reduced <- max(distances_dissimilarities_allyears.r[,jaccard_dissimilarity_nestedness_mean], na.rm = T)
...
jaccard_nestedness_model_coefs_reduced_length.unique <- unique(model_coefs_reduced_length[,.(condval,condsd, lwr, upr, survey_unit, year_adj, years_sampled, Directional_Change, hex, Survey_Name_Season, significant)]) 

#extract color hexes
#year adj coef order
color_year_adj_order <- jaccard_nestedness_model_coefs_reduced_length.unique[,hex]

#alphabetical order
jaccard_nestedness_model_coefs_reduced_length.unique.alpha <- setorder(jaccard_nestedness_model_coefs_reduced_length.unique, Survey_Name_Season)
color_alpha_order <- jaccard_nestedness_model_coefs_reduced_length.unique.alpha[,hex]

saveRDS(jaccard_nestedness_model_coefs_reduced_length.unique, here::here("output","region_stats","jaccard_nestedness_model_coefs_reduced_length.unique.Rds"))
```

Bar Plot Coefficient LMER
```{r bar plot of coefficients}
jaccard_NESTEDNESS_Dissimilarity_Coef_errorbar_reduced <- ggplot() +
  ...
  xlab("Survey unit") +
  ylab("Nestedness Jaccard dissimilarity trend") +
  coord_flip() +
  theme_classic() +
  theme(axis.text.y = element_text(colour = color_year_adj_order, face = "bold"), axis.title.y = element_blank())

jaccard_NESTEDNESS_Dissimilarity_Coef_errorbar_reduced
```
...
jaccard_nestedness_gam_predictions <- predict(jaccard_nestedness_gam, se.fit = TRUE, newdata = year_survey_unit_expand.dt)

#merge into table
year_survey_unit_expand.dt[,jaccard_glm_mod_fit := jaccard_nestedness_gam_predictions$fit][,jaccard_glm_mod_fit_SE := jaccard_nestedness_gam_predictions$se.fit]
...
points_wavylines_jaccard_nestedness_year_reduced_gam_nolmer <- ggplot() +
 # geom_ribbon(data = lmer_jaccard_nestedness_predictions, aes(x = year, ymin = jaccard_lmer_preds_lowerCI, ymax = jaccard_lmer_preds_upperCI), fill = "grey", alpha = 0.2) +
  geom_point(data = distances_dissimilarities_allyears.r,
             aes(x = year,
                 y = jaccard_dissimilarity_nestedness_mean,
 ...
 # geom_line(data = lmer_jaccard_nestedness_predictions, aes(x = year, y = jaccard_lmer_preds), color = "black") +
  ...
  lims(x = c(min(distances_dissimilarities_allyears.r[,year]),max(distances_dissimilarities_allyears.r[,year])),
       y = c(0.0,0.43)) +
  xlab("Year") +
  ylab("Nestedness Jaccard dissimilarity") +
  theme(legend.position = "null")

points_wavylines_jaccard_nestedness_year_reduced_gam_nolmer

#with lmer

points_wavylines_jaccard_nestedness_year_reduced_gam <- ggplot() +
  geom_ribbon(data = lmer_jaccard_nestedness_predictions, aes(x = year, ymin = jaccard_lmer_preds_lowerCI, ymax = jaccard_lmer_preds_upperCI), fill = "grey", alpha = 0.2) +
  geom_point(data = distances_dissimilarities_allyears.r,
             aes(x = year,
                 y = jaccard_dissimilarity_nestedness_mean,
...
points_wavylines_jaccard_nestedness_year_reduced_gam

#just one region that's homogenizing (NZ WCSI)
#Note, nestedness is not significantly homogenizing, but it is for BC balanced, so we'll keep it
points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_only <- ggplot() +
  geom_point(data = distances_dissimilarities_allyears.r[survey_unit == "NZ-WCSI",],
             aes(x = year,
                 y = jaccard_dissimilarity_nestedness_mean), alpha = 0.5, size = 1, color = jaccard_nestedness_model_coefs_reduced_length.unique[Survey_Name_Season == "W Coast S Island NZ",hex]) +
    geom_line(data = year_survey_unit_expand.dt[survey_unit == "NZ-WCSI",],
             aes(x = year,
                 y = jaccard_glm_mod_fit),
             color = jaccard_nestedness_model_coefs_reduced_length.unique[Survey_Name_Season == "W Coast S Island NZ",hex]) +
  geom_ribbon(data = year_survey_unit_expand.dt[survey_unit == "NZ-WCSI",], aes(x = year, ymin=jaccard_glm_mod_fit-jaccard_glm_mod_fit_SE, ymax=jaccard_glm_mod_fit+jaccard_glm_mod_fit_SE), alpha=0.3, fill = jaccard_nestedness_model_coefs_reduced_length.unique[Survey_Name_Season == "W Coast S Island NZ",hex]) + #add standard error
 # geom_line(data = lmer_jaccard_nestedness_predictions, aes(x = year, y = jaccard_lmer_preds), color = "black") +
...
points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_only
...
points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_Summer_only <- ggplot() +
 ...
 # geom_line(data = lmer_jaccard_nestedness_predictions, aes(x = year, y = jaccard_lmer_preds), color = "black") +
    scale_color_manual(name = "Survey Unit", values = c(jaccard_nestedness_model_coefs_reduced_length.unique[Survey_Name_Season %in% c("W Coast S Island NZ","SE US Summer"),hex])) +
  scale_fill_manual(guide = "none", values = c(jaccard_nestedness_model_coefs_reduced_length.unique[Survey_Name_Season %in% c("W Coast S Island NZ","SE US Summer"),hex])) +
    lims(x = c(min(distances_dissimilarities_allyears.r[,year]),max(distances_dissimilarities_allyears.r[,year])),
       y = c(0.0,0.43)) +
  theme_classic() +
  xlab("Year") +
...
points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_Summer_only

#just three regions, another one that's stable, (Canada Scotian Shelf)

points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_ScoShelf_only <- ggplot() +
 ...
 # geom_line(data = lmer_jaccard_nestedness_predictions, aes(x = year, y = jaccard_lmer_preds), color = "black") +
    scale_color_manual(name = "Survey Unit", values = c(jaccard_nestedness_model_coefs_reduced_length.unique[Survey_Name_Season %in% c("W Coast S Island NZ","SE US Summer","Scotian Shelf"),hex])) +
  scale_fill_manual(guide = "none", values = c(jaccard_nestedness_model_coefs_reduced_length.unique[Survey_Name_Season %in% c("W Coast S Island NZ","SE US Summer","Scotian Shelf"),hex])) +
    lims(x = c(min(distances_dissimilarities_allyears.r[,year]),max(distances_dissimilarities_allyears.r[,year])),
       y = c(0.0,0.43)) +
  theme_classic() +
  xlab("Year") +
...
points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_ScoShelf_only

ggsave(points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_only, path = here::here("figures"), filename ="points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_only.jpg", height = 5, width = 5, unit = "in")
ggsave(points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_Summer_only, path = here::here("figures"), filename ="points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_Summer_only.jpg", height = 5, width = 5, unit = "in")
ggsave(points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_ScoShelf_only, path = here::here("figures"), filename ="points_wavylines_jaccard_nestedness_year_reduced_gam_NZ_SEUS_ScoShelf_only.jpg", height = 5, width = 5, unit = "in")
points_wavylines_jaccard_nestedness_year_reduced_gam
ggsave(points_wavylines_jaccard_nestedness_year_reduced_gam_nolmer, path = here::here("figures"), filename ="points_wavylines_jaccard_nestedness_year_reduced_gam_nolmer.jpg", height = 5, width = 5, unit = "in")

ggsave(points_wavylines_jaccard_nestedness_year_reduced_gam, path = here::here("figures"), filename ="points_wavylines_jaccard_nestedness_year_reduced_gam.jpg", height = 5, width = 5, unit = "in")
...
jaccard_NESTEDNESS_GAM_LMER_merge <- plot_grid(points_wavylines_jaccard_nestedness_year_reduced_gam + theme(legend.position = "none", plot.margin = margin(2,0,0,0, unit = "cm")), jaccard_NESTEDNESS_Dissimilarity_Coef_errorbar_reduced + theme(legend.position = "none", plot.margin = margin(2,0,0,0, unit = "cm")), labels = c("","Survey Unit  ",""), vjust = 1.5, hjust = 0)

jaccard_NESTEDNESS_GAM_LMER_merge_legend <- plot_grid(jaccard_NESTEDNESS_GAM_LMER_merge, get_legend(jaccard_NESTEDNESS_Dissimilarity_Coef_errorbar_reduced), rel_widths = c(10,2))

ggsave(jaccard_NESTEDNESS_GAM_LMER_merge_legend, path = here::here("figures"), filename = "jaccard_NESTEDNESS_GAM_LMER_merge_legend.png", height = 7, width = 14, units = "in")
```
