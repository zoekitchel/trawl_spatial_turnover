...
#merge
dissimilarities_SSTtemp_fishing_area <- dates_regions.annual[dissimilarities_SSTtemp_fishing_area, on = c("survey_unit","year")]
```
...
###Full model
~ temp + survey_unit + fishing + area + latitude range + latitude average + depth range + depth average + spp count + # of hauls + julian day
...
global_mod_mean_temp <- lm(annual_dissimilarity_value ~ 
...
global_mod_max_temp <- lm(annual_dissimilarity_value ~ 
...
global_mod_min_temp <- lm(annual_dissimilarity_value ~ 
...
global_mod_seas_temp <- lm(annual_dissimilarity_value ~ 
...
global_mod_SD_temp <- lm(annual_dissimilarity_value ~ 
...
global_mod_sst_table <- global_mod_temp_table[,.(Rank,`Model structure`, deltaAICc)]
...
interaction_avg_model_coef <- model_avg_values[c(4,36:66),.(coef_name, coef, se)]
...
```{r}
model_avg_values.nonfishortemp <- model_avg_values[c(2,3,67:73),]
...
model_avg_values.nonfishortemp[,Variable := c("Depth","Species count","Number of tows","Area","Julian day","Latitude range","Latitude","Depth range","Temperature")]

#make factor with order
model_avg_values.nonfishortemp[,Variable := factor(Variable, levels = c("Temperature","Area","Julian day","Species count","Number of tows","Depth","Depth range","Latitude","Latitude range"))]
