...
Supplementary 8b for Kitchel et al. 2024 in prep taxonomic diversity manuscript.
...
dissimilarities_SSTtemp_fishing_area <- dissimilarities_SSTtemp_fishing_area[dissimilarity_metric == "jaccard_dissimilarity_index_binary",]
```
...
ggsave(sst_time_survey_facet_1_20, path = here::here("figures"), filename = "sst_time_survey_facet_1_20.jpg", height = 12, width =9)
...
interaction_avg_model_coef <- data.table(lm_interaction_coefficients_se(mod_name = model_avg_delta4, model_avg = T, SST_fish = T))

#add survey names
interaction_avg_model_coef[,survey_unit := factor(all_surveys[!all_surveys %in% c("ROCKALL", "GSL-S")])]

#add predictor names
interaction_avg_model_coef[,predictor := "Maximum temperature"]
...
```{r}
model_avg_values.nonfishortemp <- model_avg_values[c(2:4,8,72,73,105,106),]
...
model_avg_values.nonfishortemp[,Variable := c("Area","Depth","Latitude","Species count","Relative fishing catch","Latitude range","Number of tows","Depth range")]

#make factor with order
model_avg_values.nonfishortemp[,Variable := factor(Variable, levels = c("Relative fishing catch","Area","Species count","Number of tows","Depth","Depth range","Latitude","Latitude range"))]
...
#NEED TO EDIT IF WE MOVE FOWARD WITH THIS
