---
title: "Model annual dissimilarity with temperature, fishing, and survey identity (but for surface temperature)"
...
Supplementary 8b for Kitchel et al. 2023 in prep taxonomic diversity manuscript.
...
dissimilarities_SSTtemp_fishing <- fread(here::here("output","dissimilarities_SSTtemp_fishing.csv"))
#dissimilarities_temp_fishing <- fread(here::here("output","dissimilarities_temp_fishing.csv"))

#combine
dissimilarities_SSTtemp_fishing_area <- dissimilarities_SSTtemp_fishing[region_area_byyear, on = c("survey_unit","year")]


#only jaccard for these analyses
dissimilarities_SSTtemp_fishing_area <- dissimilarities_SSTtemp_fishing_area[dissimilarity_metric == "jaccard_dissimilarity_total",]
```
...
#merge
dissimilarities_SSTtemp_fishing_area <- dates_regions.r[dissimilarities_SSTtemp_fishing_area, on = c("survey_unit","year")]
```
...
all_surveys <- levels(as.factor(dissimilarities_SSTtemp_fishing_area$survey_unit))
setorder(dissimilarities_SSTtemp_fishing_area, survey_unit)

dissimilarities_SSTtemp_fishing_area[,Survey_Name_Season:=factor(Survey_Name_Season, levels = unique(dissimilarities_SSTtemp_fishing_area$Survey_Name_Season), ordered = T)]

(sst_time_survey_facet_1_20 <- ggplot(data = dissimilarities_SSTtemp_fishing_area[survey_unit %in% all_surveys[1:20] & year > 1979]) +
  labs(y = "Mean surface temperature (˚C)",  x = "Year") +
...
ggsave(sst_time_survey_facet_1_20, path = here::here("figures"), filename = "temp_time_survey_facet_1_20.jpg", height = 12, width =9)

(sst_time_survey_facet_21_34 <- ggplot(data = dissimilarities_SSTtemp_fishing_area[survey_unit %in% all_surveys[21:34] & year > 1979]) +
  labs(y = "Mean surface temperature (˚C)",  x = "Year") +
  geom_point(aes(y = as.numeric(yearly_mean_bypoint_avg), x = year), alpha = 0.3) +
  geom_smooth(aes(y = as.numeric(yearly_mean_bypoint_avg), x = year), method = "lm") +
  scale_x_continuous(breaks = ~ axisTicks(., log = FALSE)) +
...
ggsave(sst_time_survey_facet_21_34, path = here::here("figures"), filename = "sst_time_survey_facet_21_34.jpg", height = 12, width =9)

#######FISHING

dissimilarities_SSTtemp_fishing_area.cc <- dissimilarities_SSTtemp_fishing_area[complete.cases(dissimilarities_SSTtemp_fishing_area[,summed_tonnes_scaled_byreg]),]

(fishing_time_survey_facet_1_20 <- ggplot(data = dissimilarities_SSTtemp_fishing_area.cc[survey_unit %in% all_surveys[1:20] & year > 1979]) +
...
(fishing_time_survey_facet_21_34 <- ggplot(data = dissimilarities_SSTtemp_fishing_area.cc[survey_unit %in% all_surveys[c(21:34)] & year > 1979]) +
  labs(y = "Relative fishing catch",  x = "Year") +
  geom_point(aes(y = summed_tonnes_scaled_byreg, x = year), alpha = 0.3) +
  geom_smooth(aes(y = summed_tonnes_scaled_byreg, x = year), method = "lm") +
  scale_x_continuous(breaks = ~ axisTicks(., log = FALSE)) +
...
(preds_sst_mean_temp_survey_facet_1_20 <- ggplot(data = dissimilarities_SSTtemp_fishing_area[survey_unit %in% all_surveys[1:20] & year > 1979]) +
  labs(x = "Mean surface temperature (˚C)",  y = "β-diversity") +
...
ggsave(preds_sst_mean_temp_survey_facet_1_20, path = here::here("figures"), filename = "preds_sst_mean_temp_survey_facet_1_20.jpg", height = 12, width =9)

(preds_sst_mean_temp_survey_facet_21_34 <- ggplot(data = dissimilarities_SSTtemp_fishing_area[survey_unit %in% all_surveys[21:34] & year > 1979]) +
  labs(x = "Mean surface temperature (˚C)",  y = "β-diversity") +
  geom_point(aes(x = as.numeric(yearly_mean_bypoint_avg), y = annual_dissimilarity_value), alpha = 0.3) +
  geom_smooth(aes(x = as.numeric(yearly_mean_bypoint_avg), y = annual_dissimilarity_value), method = "lm") +
  facet_wrap(~Survey_Name_Season, scales= "free", ncol = 4) +
...
ggsave(preds_sst_mean_temp_survey_facet_21_34, path = here::here("figures"), filename = "preds_sst_mean_temp_survey_facet_21_34.jpg", height = 12, width =9)

#######FISHING

dissimilarities_SSTtemp_fishing_area.cc <- dissimilarities_SSTtemp_fishing_area[complete.cases(dissimilarities_SSTtemp_fishing_area[,summed_tonnes_scaled_byreg]),]

(preds_sst_mean_fishing_survey_facet_1_20 <- ggplot(data = dissimilarities_SSTtemp_fishing_area.cc[survey_unit %in% all_surveys[1:20] & year > 1979]) +
...
ggsave(preds_sst_mean_fishing_survey_facet_1_20, path = here::here("figures"), filename = "preds_sst_mean_fishing_survey_facet_1_20.jpg", height = 12, width =9)

(preds_sst_mean_fishing_survey_facet_21_34 <- ggplot(data = dissimilarities_SSTtemp_fishing_area.cc[survey_unit %in% all_surveys[c(21:34)] & year > 1979]) +
  labs(x = "Relative fishing catch",  y = "β-diversity") +
  geom_point(aes(x = summed_tonnes_scaled_byreg, y = annual_dissimilarity_value), alpha = 0.3) +
  geom_smooth(aes(x = summed_tonnes_scaled_byreg, y = annual_dissimilarity_value), method = "lm") +
  facet_wrap(~Survey_Name_Season, scales= "free", ncol = 4) +
...
ggsave(preds_sst_mean_fishing_survey_facet_21_34, path = here::here("figures"), filename = "preds_sst_mean_fishing_survey_facet_21_34.jpg", height = 12, width =9)
...
dissimilarity_covariates_dredge.dt <- dissimilarities_SSTtemp_fishing_area[,.
                  (year, survey_unit,
...
dissimilarity_covariates_dredge.dt[,season := factor(season, levels = c("Spring","Summer","Fall","Winter"))]


```

###Full model
~ temp + survey_unit + fishing + area + latitude range + latitude average + depth range + depth average + spp count + # of hauls + season + compare with and without autocorrelation term
...
global_mod_sst_table <- global_mod_temp_table[,.(Rank,`Temporal autocorrelation`,`Temperature variable`, deltaAICc)]

fwrite(global_mod_sst_table, here::here("output","global_mod_sst_table.csv"))


```

Best performing global model includes maximum temperature (centered and scaled)

Now, look at different combinations of all predictors (max temp) using dredge

Global model: global_mod_max_temp

```{r}

options(na.action = "na.fail") #  prevent fitting sub-models to different datasets
dd <- dredge(global_mod_max_temp_lm)
dd.dt <- data.table(dd)
View(dd)

#only models less than 4 delta AICc (4 models)
...
dd.dt.2.formatted <- dd.dt.2[,Rank := as.numeric(rownames(dd.dt.2))][,.(Rank,
                                ...
                                `survey_unit:yearly_max_bypoint_avg.scaledacrossall`,
                                delta,
...
best_model_sst_jaccard_table_formatted <- rbind(model_coef_se_AIC, model_avg.wide, fill = T)

#get rid of interaction coefficients
best_model_sst_jaccard_table_formatted.r <- best_model_sst_jaccard_table_formatted[,.(Rank,`(Intercept)`,area_km.scaledacrossall,
                               ...
                               season,
                               survey_unit,
                                yearly_max_bypoint_avg.scaledacrossall,
                                `summed_tonnes_scaled_byreg:survey_unit`,
                                `survey_unit:yearly_max_bypoint_avg.scaledacrossall`,
                                `R squared`,
                                delta,
...
numeric_cols <- names(best_model_sst_jaccard_table_formatted.r)[sapply(best_model_sst_jaccard_table_formatted.r, is.numeric)]

# Apply signif() only to numeric columns
best_model_sst_jaccard_table_formatted.r[, (numeric_cols) := lapply(.SD, function(x) if (is.numeric(x)) signif(x, digits = 2) else x), .SDcols = numeric_cols]

#change column names, in caption note that all variables are centered and scaled
colnames(best_model_sst_jaccard_table_formatted.r) <- c(
"Rank",                                              
"Intercept",                                     
...
fwrite(best_model_sst_jaccard_table_formatted.r,here::here("output","best_model_sst_jaccard_table_formatted.csv"))
```


Take aways
Dissimilarity increases with increasing avg # tows
Dissimilarity decreases with increasing avg latitude
Dissimilarity increases with increasing latitude range
Dissimilarity increases with increasing fishing effort
Dissimilarity decreases with increasing depth range
Dissimilarity  increases with increasing average depth
Dissimilarity increases with increasing area
Dissimilarity  decreases with increasing maximum temperature
...
#merge plots
predicted_values_sst_jaccard_fishing_merge <- cowplot::plot_grid(predicted_values_temp_fishing,
                                                 predicted_values_temp_fishing_meantempfishinginsurvey,
                                                 predicted_values_temp_fishing_meantempfishing,
                                                 ncol = 1)

ggsave(predicted_values_sst_jaccard_fishing_merge, path = here::here("figures"), filename = "predicted_values_sst_jaccard_fishing_merge.jpg", height = 30, width = 14)
```
...
#merge
jaccard_fishing_sst_model_observed_predicted_merge <- plot_grid(jaccard_fishing_temp_model_observed_predicted + theme(plot.margin = unit(c(0.1,0.3,0.1,0.1),"cm")),
...
ggsave(jaccard_fishing_sst_model_observed_predicted_merge, path = here::here("figures"),filename = "jaccard_fishing_sst_model_observed_predicted_merge.jpg", height =6, width = 8)
...
interaction_avg_model_coef <- model_avg_values[c(7,39:69),.(coef_name, coef, se)]

#insert AI
interaction_avg_model_coef[1,survey_unit := "AI"][1,coef_true := coef][1,predictor := c("Relative fishing catch")]

#Other summed_tonnes_scaled_byreg
interaction_avg_model_coef[2:32,survey_unit := substr(coef_name, 39, str_length(coef_name))][2:32,coef_true := interaction_avg_model_coef[1,coef_true]+coef][2:32,predictor := "Relative fishing catch"]
...
```{r}
model_avg_values.nonfishortemp <- model_avg_values[c(2,6,70:75),]
...
model_avg_values.nonfishortemp[,Variable := c("Depth","Species count","Number of tows","Area","Latitude range","Depth range","Latitude","Temperature")]

#make factor with order
model_avg_values.nonfishortemp[,Variable := factor(Variable, levels = c("Temperature","Area","Species count","Number of tows","Depth","Depth range","Latitude","Latitude range"))]
...
model_coef_summary_sst_jaccard <- cowplot::plot_grid(fishing_avg_model_coef+theme(legend.position = "null", axis.title.x = element_blank()),all_avg_model_coef+theme(legend.position = "null"), ncol = 1, labels = c("a.","b."), label_y = 0.99, rel_heights = c(3,1))

ggsave(model_coef_summary_sst_jaccard, path = here::here("figures"),filename = "model_coef_summary_sst_jaccard.jpg", height = 6.5, width = 8, unit = "in")
...
model_intercept_jaccard_sst <- cowplot::plot_grid(survey_model_coef+theme(axis.title.x = element_blank()),
                                                     season_model_coef, ncol = 1, labels = c("a.","b."), label_y = 0.99, rel_heights = c(3,1), align = "v")

ggsave(model_intercept_jaccard_sst, path = here::here("figures"),filename = "model_intercept_jaccard_sst.jpg", height = 6.5, width = 6.5, unit = "in")
