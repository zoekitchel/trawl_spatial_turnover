...
interaction_avg_model_coef[,predictor := factor(predictor, levels = c("Relative fishing catch"))]
...
```{r}
model_avg_values.nonfishortemp <- model_avg_values[c(2,3,67:72),]
...
model_avg_values.nonfishortemp[,Variable := c("Depth","Species count","Number of tows","Area","Latitude range","Latitude","Depth range","Maximum temperature")]

#make factor with order
model_avg_values.nonfishortemp[,Variable := factor(Variable, levels = c("Maximum temperature","Area","Species count","Number of tows","Depth","Depth range","Latitude","Latitude range"))]
...
model_coef_summary_sst_jaccard <- cowplot::plot_grid(fishing_avg_model_coef+theme(legend.position = "null", axis.title.x = element_blank()),all_avg_model_coef+theme(legend.position = "null"), ncol = 1, labels = c("a.","b."), label_y = 0.99, rel_heights = c(2,1))
