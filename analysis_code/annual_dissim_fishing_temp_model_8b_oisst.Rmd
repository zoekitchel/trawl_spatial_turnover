...
#combine
dissimilarities_temp_fishing_area <- dissimilarities_SSTtemp_fishing[region_area_byyear, on = c("survey_unit","year")]
```
...
Plot temperature vs. dissimilarity for all regions
```{r}
...
(preds_sst_mean_temp_survey_facet_1_24 <- ggplot(data = dissimilarities_temp_fishing_area[survey_unit %in% all_surveys[1:24]]) +
...
ggsave(preds_sst_mean_temp_survey_facet_1_24, path = here::here("figures"), filename = "preds_sst_mean_temp_survey_facet_1_24.jpg", height = 12, width =9)

(preds_sst_mean_temp_survey_facet_25_42 <- ggplot(data = dissimilarities_temp_fishing_area[survey_unit %in% all_surveys[25:42]]) +
  labs(x = "Mean surface temperature (˚C)",  y = "β-diversity") +
  geom_point(aes(x = as.numeric(yearly_mean_bypoint_avg), y = bray_curtis_dissimilarity_total_mean), alpha = 0.3) +
  geom_smooth(aes(x = as.numeric(yearly_mean_bypoint_avg), y = bray_curtis_dissimilarity_total_mean), method = "lm") +
  facet_wrap(~Survey_Name_Season, scales= "free", ncol = 4) +
...
ggsave(preds_sst_mean_temp_survey_facet_25_42, path = here::here("figures"), filename = "preds_sst_mean_temp_survey_facet_25_42.jpg", height = 12, width =9)
```
...
best_SSTmodel_table_formatted <- rbind(model_coef_se_AIC, model_avg.wide, fill = T)

#get rid of interaction coefficients
best_SSTmodel_table_formatted.r <- best_SSTmodel_table_formatted[,.(Rank,`(Intercept)`,area_km.scaledacrossall,
...
numeric_cols <- names(best_SSTmodel_table_formatted.r)[sapply(best_SSTmodel_table_formatted.r, is.numeric)]

# Apply signif() only to numeric columns
best_SSTmodel_table_formatted.r[, (numeric_cols) := lapply(.SD, function(x) if (is.numeric(x)) signif(x, digits = 2) else x), .SDcols = numeric_cols]

#change column names, in caption note that all variables are centered and scaled
colnames(best_SSTmodel_table_formatted.r) <- c(
"Rank",                                              
"Intercept",                                     
...
fwrite(best_SSTmodel_table_formatted.r,here::here("output","best_SSTmodel_table_formatted.csv"))
```
...
Dissimilarity   decreases with increasing mean temperature
...
dissimilarity_covariates_dredge.dt_predictions <- copy(dissimilarity_covariates_dredge.dt)
  ...
  dissimilarity_covariates_dredge.dt_predictions_consistenttempinreg[,yearly_mean_bypoint_avg.scaledacrossall:=mean(yearly_mean_bypoint_avg.scaledacrossall),.(survey_unit)]
...
dissimilarity_covariates_dredge.dt_predictions_consistenttempfishinginreg[,summed_tonnes_scaled_byreg:=mean(summed_tonnes_scaled_byreg),.(survey_unit)][,yearly_mean_bypoint_avg.scaledacrossall:=mean(yearly_mean_bypoint_avg.scaledacrossall),.(survey_unit)]
...
dissimilarity_covariates_dredge.dt_predictions_consistenttempfishing[,summed_tonnes_scaled_byreg:=mean(summed_tonnes_scaled_byreg)][,yearly_mean_bypoint_avg.scaledacrossall:=mean(yearly_mean_bypoint_avg.scaledacrossall)]
...
#merge plots
predicted_values_SSTtemp_fishing_merge <- cowplot::plot_grid(predicted_values_temp_fishing,
                                                 predicted_values_temp_fishing_meantempfishinginsurvey,
                                                 predicted_values_temp_fishing_meantempfishing,
                                                 ncol = 1)

ggsave(predicted_values_SSTtemp_fishing_merge, path = here::here("figures"), filename = "predicted_values_SSTtemp_fishing_merge.jpg", height = 30, width = 14)
```
...
Merge
```{r}
BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_predicted_merge_SST <-
...
ggsave(BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_predicted_merge_SST, path = here::here("figures"),filename = "BC_total_Dissimilarity_Coef_errorbar_reduced_colorbytrend_predicted_merge_SST.jpg", height = 20, width = 8)
```
...
#fishing constant (temperature only)
...
summary(bray_curtis_fishing_maintained_model_observed_predicted_lm) #R^2 = 0.17
...
summary(bray_curtis_temp_maintained_model_observed_predicted_lm) #0.49
...
summary(bray_curtis_fishing_temp_model_observed_predicted_tempfishconstantinsurvey_lm) #0.16

#merge
bray_curtis_fishing_SSTtemp_model_observed_predicted_merge <- plot_grid(bray_curtis_fishing_temp_model_observed_predicted,
...
ggsave(bray_curtis_fishing_SSTtemp_model_observed_predicted_merge, path = here::here("figures"),filename = "bray_curtis_fishing_SSTtemp_model_observed_predicted_merge.jpg", height = 8, width = 8)
