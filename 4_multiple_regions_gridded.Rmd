...
library(data.table)
```
Playing around with dggridR
```{r learning dggridR}
dggs <- dgconstruct(spacing = 1000, metric = F, resround = 'down')
data("dgquakes")
dgquakes <- data.table(dgquakes)

dgquakes[,cell := dgGEO_to_SEQNUM(dggs, lat, lon)] #get corresponding grid cells for each earquake epicenter

quakcounts <- dgquakes[, .N, by = cell]
```
Bring in our maps first for USA regions
