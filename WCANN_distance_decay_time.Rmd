...
wcann_similarity_mod_coefs <- data.table("year" = wcann_years, "jaccard_final_sim" = 0, "jaccard_alpha" = 0, "bray_final_sim" = 0, "bray_alpha" = 0)

for (i in 1:length(wcann_years)) {
  # Estimate the parameters using a linear model for this year
  fit_bray <- nls(bray_curtis_similarity ~ SSasymp(`distance(km)`, yf, y0, log_alpha), data = distances_dissimilarities_allyears_CELL[year == wcann_years[i]])
  # I() allows - to be used arithmetically, here we get similarity versus distance models
  
  wcann_similarity_mod_coefs[i, bray_final_sim :=  coef(fit_bray)[1]]
  wcann_similarity_mod_coefs[i, bray_alpha :=  coef(fit_bray)[3]]
  
  fit_jaccard <- nls(jaccard_similarity ~ SSasymp(`distance(km)`, yf, y0, log_alpha), data = distances_dissimilarities_allyears_CELL[year == wcann_years[i]])
  
  wcann_similarity_mod_coefs[i, jaccard_final_sim :=  coef(fit_jaccard)[1]]
  wcann_similarity_mod_coefs[i, jaccard_alpha :=  coef(fit_jaccard)[3]]
...
›ggsave(path = "plots", file = "slopes_for_distance_decay_wcann.jpg", width = 6, height = 4, unit = "in")
