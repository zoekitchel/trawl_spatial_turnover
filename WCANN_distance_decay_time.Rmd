---
title: "WCANN Distance Decay over Time"
...
wcann_data <- readRDS("wcann_data_gridded.RData")
...
wcann_distances.l[,`distance(km)` := `distance(m)`/1000][,`distance(m)` := NULL]
```

I need it to be a data frame formatted species (rows) by sites/years (columns)

```{r sum across grid cells and years}
wcann_data_year_cell <- wcann_data[,wtcpue_sum := sum(wtcpue, na.rm = T), .(matched_name2, cell, year)]

wcann_data_year_cell.r <- unique(wcann_data_year_cell[,.(matched_name2, cell, year, wtcpue_sum)])
```
...
#list of years 
wcann_years <- unique(wcann_data_year_cell.r[,year])

sort(wcann_years)

i = 1

wcann_subset_year <- wcann_data_year_cell.r[year == wcann_years[i],]

#long to wide
wcann_subset_year_wide <- dcast(wcann_subset_year, cell ~ matched_name2, value.var = "wtcpue_sum", fill = 0)
...
for (i in 1:length(wcann_years)) {
  reduced_year <- wcann_data_year_cell.r[year == wcann_years[i],]
  ...
  dissimilarities_full <- wcann_distances.l[dissimilarities_abundance.l, on = c("cell1", "cell2")]
...
save(distances_dissimilarities_allyears_CELL, file = "wcann_bray_jaccard_similarities_distances_cell.Rdata")
...
wcann_similarity_mod_coefs <- data.table("year" = wcann_years, "jaccard_beta" = 0, "bray_beta" = 0)

for (i in 1:length(wcann_years)) {
  # Estimate the parameters using a linear model for this year
  mod1 <- lm(I(1 - bray_curtis_dissimilarity) ~ `distance(km)`, data = distances_dissimilarities_allyears_CELL[year == wcann_years[i],])  # I() allows - to be used arithmetically, here we get similarity versus distance models
  
  wcann_similarity_mod_coefs[i, bray_beta :=  -coef(mod1)[2]] #change in similarity for change in distance
  
  mod2 <- lm(I(1 - jaccard_dissimilarity) ~ `distance(km)`, data = distances_dissimilarities_allyears_CELL[year == wcann_years[i],])
  
  wcann_similarity_mod_coefs[i, jaccard_beta :=  -coef(mod2)[2]]

}

ggplot(aes(x = as.numeric(year)), data = wcann_similarity_mod_coefs) +
  geom_point(aes(y = jaccard_beta), shape = 15) +
  geom_smooth(aes(y = jaccard_beta), linetype = 1, color = "black") +
  geom_point(aes(y = bray_beta), shape = 4) +
  labs(x = "Year", y = "Slope of Distance Decay Plot\n(ΔSimilarity/ΔDistance (km))") +
  geom_smooth(aes(y = bray_beta), linetype = 2, color = "black") +
  theme_classic() +
  theme(text = element_text(size = 13))

ggsave(path = "plots", file = "wcann_slopes_for_distance_decay.jpg", width = 6, height = 4, unit = "in")
