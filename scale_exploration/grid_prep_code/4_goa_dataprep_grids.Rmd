...
#this spp scored randomly low
#-Laqueus californicus

#delete repeats and get weird of weird ones


result.short <- result.long[result.long[, .I[which.max(score)], by=.(user_supplied_name, matched_name2)]$V1][,spp := user_supplied_name][score > 0.75 | matched_name2 == "Laqueus californicus",]
...
cell_numbers <- unique(dat_GOA.clean$cell)
years <- unique(dat_GOA.clean$year)
seasons <- unique(dat_GOA.clean$region)

cells_cross_years_cross_region <- data.table(expand.grid(cell = cell_numbers, year = years, region = seasons))

observations <- unique(dat_GOA.clean[,.N,by = c("year","cell", "region")])

#merge possible cell year region combinations with actual observations
goa_cell_data_nodata <- observations[cells_cross_years_cross_region, on = .(year, cell, region)]
...
dat_GOA_grid.reduced <- dat_GOA.clean[!cell %in% dump]
dim(dat_GOA_grid.reduced )
#1840756 observations left
...
haul_cell_unique <- unique(dat_GOA_grid.reduced[,.(haulid, cell, year)])
...
dat_goa.reduced.again <- dat_GOA_grid.reduced[!cell %in% cells_to_delete]
...
goa_grid_cells <- unique(dat_GOA_grid.reduced[,.(cell, LON_CENTER, LAT_CENTER)])
...
goa_unique_tows_cell <- dat_GOA_grid.reduced[,.SD[1],.(year, cell, haulid)]
...
dat_goa_grid.reduced_3plustows <- dat_GOA_grid.reduced[goa_keep, on = .(cell, year)]
...
Save
```{r save}
saveRDS(dat_GOA_grid.reduced, "/Users/zoekitchel/Documents/grad school/Rutgers/Repositories/trawl_spatial_turnover/data/gridded/USA-GOA/dat_GOA_grid.reduced.rds")
