...
```{r clean up}
neus_full.ID <- neus_full[SVSPP != 0,] # TO DO: need to trim out other categories, like TRASH here
neus_full.ID2 <- neus_full.ID[ , .(BIOMASS = sum(BIOMASS)), by = .(CRUISE6, STATION, STRATUM, TOW, SVSPP, YEAR, SEASON, LAT, LON)]
...
### Save out
```{r save}
save(neus_full.ID2, file = "data/NE_trawl_data_full.Rdata")
```
