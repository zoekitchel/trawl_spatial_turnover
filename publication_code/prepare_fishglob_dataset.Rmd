...
FishGlob_1.5[survey == "ZAF" & longitude <20.01, survey_unit := "ZAF-ATL"][survey == "ZAF" & longitue >= 20.01, survey_unit := "ZAF-IND"]
```
...
gmex_fall_hauls_keep <- unique(FishGlob.10year.uniquehauls[survey_unit == "GMEX-Fall" & month %in% c(9,10,11), haul_id])
```
...
In the Northeast U.S., we excluded data from years prior to 1982 (data begin in 1968).  Years prior to 1979 were excluded because strata in the southern tip of the region (between  approximately 34.5º N and 35º N) were not regularly sampled during this time. (Batt et al. 2017)
...
-Richness higher in second half of time series, so we will limit to >=2000
-From 2000, consistent number of hauls except for 2013 which we will exclude


```{r Nor-BTS-1 processing}
nor_bts_1_keep <- unique(FishGlob.10year.uniquehauls[survey_unit == "Nor-BTS-1" & month %in% c(1:3) & year >= 2000 & year != 2013, haul_id])
```
...
-Use months 2,3,4
-Inconsistent spatial distribution through time (northern latitudes only sampled in early years), but this should be fixed in spatial standardization procedure
-Seemingly consistent richness
-Number of hauls is variable, exclude super low and high numbers (1985,1994,2015,2019)


```{r scs_spring processing}
scs_spring_keep <- unique(FishGlob.10year.uniquehauls[survey_unit == "SCS-SPRING" & month %in% c(2,3,4) & !(year %in% c(1985,1994,2015,2019)), haul_id])
```
...
###SEUS
- In the Southeast U.S., data from 1989 (data begin in 1989) were excluded because several  sites were not sampled in this year, and if this year had been used, the number of sites sampled in  at least 85% of years would drop from 24 to 23 (with only 21 sites sampled in 1989). (Batt et al. 2017)
...
```{r ZAF}
ggplot(FishGlob.10year.uniquehauls.season[survey_unit == "ZAF",]) +
  geom_tile(aes(x = factor(month), y = factor(season), fill = haul_proportion_survey_season),color = "white") +
  scale_fill_viridis() +
  labs(x = "Month", y = "Season",fill = "Proportion of All Survey Hauls in FishGlob") +
  theme_classic()

ggplot(FishGlob.10year.uniquehauls.quarter[survey_unit == "ZAF",]) +
  geom_tile(aes(x = factor(month), y = factor(quarter), fill = haul_proportion_survey_quarter),color = "white") +
  scale_fill_viridis() +
  labs(x = "Month", y = "Quarter",fill = "Proportion of All Survey Hauls in FishGlob") +
  theme_classic()

ggplot(FishGlob.10year.uniquehauls.annual.month[survey_unit == "ZAF",]) +
  geom_tile(aes(x = year, y = factor(month), fill = haul_proportion_month_yearly),color = "white") +
  scale_fill_viridis() +
  labs(x = "Year", y = "Month",fill = "Proportion of Annual Hauls") +
  theme_classic()

ggplot(FishGlob.10year.uniquehauls.annual.quarter[survey_unit == "ZAF",]) +
  geom_tile(aes(x = year, y = factor(quarter), fill = haul_proportion_quarter_yearly),color = "white") +
  scale_fill_viridis() +
  labs(x = "Year", y = "Quarter",fill = "Proportion of Annual Hauls") +
  theme_classic()

ggplot(FishGlob.10year.uniquehauls[survey_unit == "ZAF",]) +
  geom_point(aes(x = longitude_adj, y = latitude, color = year), size = 0.3, alpha = 0.5) +
  scale_color_viridis() +
  theme_classic()
...
# # of species per year??
ggplot(data = FishGlob.10year.spp_survey_year.r[survey_unit == "ZAF",], aes(x = year, y=spp_count_survey_year)) +
  geom_col() +
  theme_classic()

# # of hauls per year
ggplot(data = FishGlob.10year.spp_haulid_year.r[survey_unit == "ZAF",], aes(x = year, y=haulid_count_survey_year)) +
  geom_col() +
  theme_classic()
...
```{r zaf processing}
zaf_keep <- unique(FishGlob.10year.uniquehauls[survey_unit == "ZAF" & month %in% c(6:9), haul_id])
```
