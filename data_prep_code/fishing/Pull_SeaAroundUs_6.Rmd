...
#unique years and surveys between 1979 and 2018 (offset by one as a predictor)
year_survey_match <- unique(FishGlob_clean.singleseason[year > 1978 & year < 2020, .(survey_unit, year)])

#limit catch data
SAU_summed_tonnes.r <- SAU_summed_tonnes.u[year_survey_match, on = c("survey_unit","year")]
```
...
SAU_summed_tonnes.r <- unique(SAU_summed_tonnes_full[,.(year, survey_unit, summed_tonnes, summed_tonnes_scaled, summed_tonnes_scaled_byreg)])
