...
SAU_summed_tonnes <- SAU_full.landings.ided[,summed_tonnes := sum(tonnes, na.rm = T),.(year, area_name, area_type, Survey)][ #key to include Survey because a few SAU data sets are in here multiple times
    , summed_tonnes_scaled_byreg := scale(summed_tonnes),.(Survey)]

#reduce to unique values we need
SAU_summed_tonnes.r <- unique(SAU_summed_tonnes[,.(year, Survey, summed_tonnes, summed_tonnes_scaled_byreg)])
...
#save
saveRDS(SAU_summed_tonnes.r, here::here("data","sea_around_us","SAU_summed_tonnes.r"))
```
