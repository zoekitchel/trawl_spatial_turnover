...
Pull together sea around us data from the web by closets EEZ

This code is Script 6 for Kitchel et al. TITLE manuscript.
...
SAU_summed_tonnes <- SAU_full.landings.ided[,summed_tonnes := sum(tonnes, na.rm = T),.(year, area_name, area_type, Survey)][ #key to include Survey because a few SAU data sets are in here multiple times
    , summed_tonnes_scaled_byreg := scale(summed_tonnes),.(Survey)][#scale by region (ie high relative to given region)
...
ggplot(SAU_summed_tonnes.r) +
  geom_line(aes(x = year, y = summed_tonnes_scaled_byreg, color = Survey)) +
  theme_classic()
