...
for (i in 1:length(all_survey_seasons)) {
  ...
  benchmark_70 <- 0.7 * max(year_cells[,cell_count])
  ...
  print(ifelse(length(cell_id_remove) == 0, paste0(all_survey_seasons[i], " Cells left out = 0"), paste0(all_survey_seasons[i], " Cells left out = ", cell_id_remove, ", ",cells_deleted_percent, "% Cells Excluded, ",obs_removed, "% Observations Removed" ))) #need to figure out how to list all cells removed
