...
Match lat/lon sampling points to hexagonal cells, so that we can see how many cells to keep to maintain a high portiion of observation points
...
for (i in 1:length(all_survey_units)) {
      ...
      #new column calculate hauls per year
      FishGlob.cells.subset[,hauls_per_year := uniqueN(haul_id),.(year,survey_unit)]
    ...
    #plot years that are removed
     fishglob_exclude_years_plots[[i]] <- 
       ggplot() +
  geom_col(data = unique(FishGlob.cells.subset[,.(year, survey_unit, `Excluded year`,hauls_per_year)]), aes(x = year, y = hauls_per_year, fill = `Excluded year`)) +
       labs(x = "Year",y = "Unique tows per year") +
       scale_fill_manual(values = c("grey","turquoise")) +
  theme_classic() +
       ggtitle(paste0(all_survey_units[i], ", ",percent_hauls_removed_year, "% tows excluded" ))
  ...
  cell_remove_polygon_sf <- sf::st_as_sf(cell_remove_polygon, coords = c("x","y"))
      ...
      fishglob_exclude_cells_plots[[i]] <- 
       ...
       ggtitle(paste0(all_survey_units[i], ", ",hauls_removed_yearcell, "% tows excluded" ))
...
#only keep survey units for which fewer than 50% of hauls were deleted by 70% thresholds
survey_units_keep <- unique(data_loss[percent_hauls_excluded_total <= 50,survey_unit])
...
data_loss.final <- data_loss[,.(Survey_Name_Season, year_threshold, percent_years_excluded, percent_hauls_excluded_by_year, cell_threshold, percent_cells_excluded, percent_hauls_excluded_total)]

colnames(data_loss.final)<-c("Survey", "Year inclusion threshold\n(% cells sampled in year)", "Percent years excluded", "Percent hauls excluded\nfrom year standardization","Cell inclusion threshold\n(% years sampled in cells)", "Percent cells excluded", "Total percent hauls excluded")
