...
#missing values
#missing abundances
180786/nrow(FishGlob_clean) #7% missing abundances

#missing biomass
357932/nrow(FishGlob_clean) #13% missing biomass


```
Visualize standardization proceedure
```{r}
merged_ebs_standardization_plot <- cowplot::plot_grid(fishglob_exclude_years_plots[[36]]+theme(legend.position = "null"), fishglob_exclude_cells_plots[[36]], ncol = 1, labels = c("a.","b."), rel_heights = c(1,1.3))

ggsave(merged_ebs_standardization_plot, path = here::here("figures","standardization"),filename ="merged_ebs_standardization_plot.jpg", height = 6.5, width = 6.5, unit = "in")
```
...
unique(FishGlob_clean.singleseason$survey_unit)
