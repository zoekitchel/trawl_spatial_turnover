...
for (i in 1:length(years)) {
  ...
  mod <- lm(I(1 - jaccard_dissimilarity) ~ `distance(m)`, data = distances_dissimilarities_allyears_STRATUM[YEAR == years[i],])  
  mod_coefs[i, beta :=  -coef(mod)[2]]

}

invisible(mod_coefs[ , plot(year, beta)])
invisible(mod_coefs[, abline(lm(beta ~ year))])
