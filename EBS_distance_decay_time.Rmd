...
for (i in 1:length(ebs_years)) {
  reduced_year <- ebs_data_year_cell.r[year == ebs_years[i],]
...
ebs_similarity_mod_coefs <- data.table("year" = ebs_years, "jaccard_beta" = 0, "bray_beta" = 0)

for (i in 1:length(ebs_years)) {
  # Estimate the parameters using a linear model for this year
  mod1 <- lm(I(1 - bray_curtis_dissimilarity) ~ `distance(km)`, data = distances_dissimilarities_allyears_CELL[year == ebs_years[i],])  # I() allows - to be used arithmetically, here we get similarity versus distance models
  
  ebs_similarity_mod_coefs[i, bray_beta :=  -coef(mod1)[2]] #change in similarity for change in distance
  
  mod2 <- lm(I(1 - jaccard_dissimilarity) ~ `distance(km)`, data = distances_dissimilarities_allyears_CELL[year == ebs_years[i],])
  
  ebs_similarity_mod_coefs[i, jaccard_beta :=  -coef(mod2)[2]]
