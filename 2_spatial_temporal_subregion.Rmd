...
reg_centroids <- reg_centroids[, 10:12] #we only need subregion, centroid lon and centroid lat columns
...
#the location of MAB still doesn't make sense (too far south), but I'm moving forward anyway
```
...
merge[,"present" := ifelse(is.na(SCINAME), 0,1)][,"SCINAME" := NULL][,"COMNAME" := NULL] #if scientific name and common name are empty, that is an absence (0), if populated, presence (1), also, we don't need these columns beyond presence/absence ID, as we'll bring them in below

#spp key
spp_key <- unique(neus_full.ID[,c("SVSPP", "SCINAME", "COMNAME")], col = c("SVSPP", "SCINAME", "COMNAME"))
...
ggsave(geo_gme_overtime, file ="plots/GEO & GME Jaccard Through Time.eps")
```
...
ggsave(geo_sne_overtime, file ="plots/GEO & SNE Jaccard Through Time.eps")
```
...
ggsave(gme_mab_overtime, file ="plots/GME & MAB Jaccard Through Time.eps")
```
...
ggsave(sne_geo_overtime, file ="plots/SNE & GEO Jaccard Through Time.eps")
```
...
ggsave(mab_geo_overtime, file ="plots/MAB & GEO Jaccard Through Time.eps")
...
ggsave(jaccard_overtime_GME_year1comparison, file ="plots/jaccard_overtime_GME_year1comparison.eps" )
...
ggsave(jaccard_overtime_GEO_year1comparison, file ="plots/jaccard_overtime_GEO_year1comparison.eps" )
...
ggsave(jaccard_overtime_SNE_year1comparison, file ="plots/jaccard_overtime_SNE_year1comparison.eps" )
...
ggsave(jaccard_overtime_MAB_year1comparison, file ="plots/jaccard_overtime_MAB_year1comparison.eps" )
...
ggsave(species_richness_reg, file ="plots/species_richness_reg.eps")
