...
#delete repeats
neus_fall_distances_dissimilarities_allyears <- neus_fall_distances_dissimilarities_allyears[haulid1 >= haulid2,] #3165272 to 1588479 rows
...
neus_fall_distances_dissimilarities_allyears_jaccard_rounded[,year:=as.numeric(year)]
...
#jaccard similarity
neus_fall_jaccard_heatmap <- ggplot(neus_fall_distances_dissimilarities_allyears,aes(as.factor(year),distance/1000, color = jaccard_similarity)) +
  geom_point(shape = 15, size = 0.7) +
  scale_color_gradientn(colors =
c("purple" ,"blue"  , "green" , "yellow" ,"orange", "red")
  ) +
scale_x_discrete(limits=levels(neus_fall_distances_dissimilarities_allyears$year.f), breaks = c("1972", "1980", "1990", "2000", "2010", "2019")) +
...
neus_fall_bray_heatmap <- ggplot(neus_fall_distances_dissimilarities_allyears,aes(as.factor(year),distance/1000, color = bray_curtis_similarity)) +
  geom_point(shape = 15, size = 0.7) +
  scale_color_gradientn(colors =
c("purple" ,"blue"  , "green" , "yellow" ,"orange", "red")
  ) +
...
#jaccard similarity
(neus_fall_jaccard_boxplot <- ggplot(neus_fall_distances_dissimilarities_allyears,aes(year.f,jaccard_similarity)) +
...
#bray curtis similarity
(neus_fall_bray_boxplot <- ggplot(neus_fall_distances_dissimilarities_allyears,aes(year.f,bray_curtis_similarity)) +
  geom_boxplot(outlier.shape = NA, lwd = 0.2) +
...
neus_fall_avg_jaccard_mod <- lm(data = neus_fall_distances_dissimilarities_allyears, formula = jaccard_similarity ~ as.numeric(year))

neus_fall_avg_bray_curtis_mod <- lm(data = neus_fall_distances_dissimilarities_allyears, formula = bray_curtis_similarity ~ as.numeric(year))
...
neus_fall_merge <- plot_grid(neus_fall_jaccard_heatmap + theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank()) , neus_fall_bray_heatmap + theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank()), neus_fall_legend_heatmaps, neus_fall_jaccard_boxplot, neus_fall_bray_boxplot,  nrow = 2, ncol = 3, align = "hv", axis = "lb")

neus_fall_merge

ggsave(neus_fall_merge, path = here::here("figures","neus"), file = "neus_fall_heat_boxplot_merge.jpg", width = 8, height = 5, unit = "in")
ggsave(neus_fall_merge, path = here::here("figures","neus"), file = "neus_fall_heat_boxplot_merge.eps", width = 8, height = 5, unit = "in")
...
estimate_jac_sim_by_year <- function(target_year, target_distance) {
  data_for_year <- neus_fall_distances_dissimilarities_allyears[year == target_year]
...
temp_interp_depth <- data.table(expand.grid(
  # the same years as neus fall
  year = neus_years_fall,
  # ddistances can now be any value
  distance = seq(0, max(neus_fall_distances_dissimilarities_allyears$distance), by = 10))
)
...
jac_sim_raster <- data.table(expand.grid(
  ...
  year = seq(min(neus_years_fall), max(neus_years_fall), by = 1),
