...
Haul ID keys
```{r list of haulids}
wctri_haulids <- unique(dat_WCTRI_grid.reduced_3plustows[,haulid])
wctri_haulids_key <- data.table(haulid = wctri_haulids, key_ID = seq(1,length(wctri_haulids), by = 1))
...
dat_WCTRI_grid.reduced_3plustows <- dat_WCTRI_grid.reduced_3plustows[wctri_haulids_key, on = "haulid"]
...
wctri_distances_dissimilarities_allyears <- data.table("haulid1" = integer(), "haulid2" = integer(), "distance" = numeric(),"bray_curtis_dissimilarity" = numeric(), year = integer(), "jaccard_dissimilarity" = numeric())
...
for (i in 1:length(wctri_years)) {
  ...
  wctri_distances <- distm(lat_lon_haulid[,.(lon,lat)])
  key_IDs <- lat_lon_haulid[,key_ID]

  colnames(wctri_distances) <- rownames(wctri_distances) <- key_IDs

  #wide to long
  haulid_distances.l <- reshape2::melt(wctri_distances,varnames = (c("haulid1", "haulid2")), value.name = "distance")
...
#delete repeats
wctri_distances_dissimilarities_allyears <- wctri_distances_dissimilarities_allyears[haulid1 >= haulid2,] #3165272 to 1588479 rows
...
wctri_merge <- plot_grid(wctri_jaccard_heatmap + theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank()) , wctri_bray_heatmap + theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank()), wctri_legend_heatmaps, wctri_jaccard_boxplot, wctri_bray_boxplot,  nrow = 2, ncol = 3, align = "hv", axis = "lb")

wctri_merge

ggsave(wctri_merge, path = here::here("figures","wctri"), file = "wctri_heat_boxplot_merge.jpg", width = 8, height = 5, unit = "in")
ggsave(wctri_merge, path = here::here("figures","wctri"), file = "wctri_heat_boxplot_merge.eps", width = 8, height = 5, unit = "in")
