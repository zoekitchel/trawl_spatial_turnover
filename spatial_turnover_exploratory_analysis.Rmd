...
neus_full.ID.regions <- neus_full.ID[region_strata_key.r, on = "STRATUM_NAME"] #this got rid of all rows without a reg
summary(neus_full.ID.regions$reg)
...
#the location of MAB still doesn't make sense, but I'm moving forward anyway, much better than before
```
...
similarities_1963.l <- melt(as.matrix(similarities_1963), varnames = c("reg1", "reg2"))
similarities_1963.l <- data.table(similarities_1963.l) #and then to data table
similarities_1963.l <- similarities_1963.l[reg1 >= reg2,] # to get rid of repetitions
colnames(similarities_1963.l)[colnames(similarities_1963.l)=="value"] <- "jaccard_similarity" #change column name to distance


similarities_1963.l[reg1 == reg2, jaccard_similarity := 1] #if they are the same value, assign jaccard_similarity of one

#add year for these values
similarities_1963.l[, "YEAR" := 1963]

#merge distance with jaccard_similarity for this year
dissimilarity_1963 <- reg_distance.l[similarities_1963.l, on = c("reg1", "reg2")]

#make plot
plot(x=dissimilarity_1963$`distance(m)`, y=dissimilarity_1963$jaccard_similarity) #this doesn't make sense, going back
