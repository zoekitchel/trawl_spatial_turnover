...
name_key <- data.table(survey_unit = c("NZ-WCSI","SEUS-summer","SCS-SUMMER"),`Survey Unit` = c("W Coast S Island, NZ","SE US","Scotian Shelf, CA"))

full_mds.dt <- full_mds.dt[name_key, on = "survey_unit"]

#change factor order to match other components of manuscript
full_mds.dt[,`Survey Unit` := factor(`Survey Unit`, levels = c("W Coast S Island, NZ","SE US","Scotian Shelf, CA"))]
...
mds_contrast <- ggplot(data = full_mds.dt) +
  ...
  scale_color_manual(values = c("#91c874","#e7ac5b", "#cbbfde")) +
  theme_classic() +
