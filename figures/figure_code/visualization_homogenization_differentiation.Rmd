...
Example of homogenization and differentiation as ODS

This code is Script for Figure X for Kitchel et al. TITLE manuscript.
...
seus_years <- unique(FishGlob_clean.noNA[survey_unit == "SEUS-summer",year])
#year 4 = 1993,  final year = 2019

seus_summer <- FishGlob_clean.noNA[survey_unit == "SEUS-summer" & year %in% c(1993,2019),]
...
seus_summer_mds_contrast <- ggplot(data = seus_full_mds.dt) +
  geom_point(aes(x = NMDS1, y = NMDS2), color = "#FCE100") +
...
```{r}
nz_wcsi_years <- unique(FishGlob_clean.noNA[survey_unit == "NZ-WCSI",year])
#year 1 = 2000,  final year = 2019

nz_wcsi <- FishGlob_clean.noNA[survey_unit == "NZ-WCSI" & year %in% c(2000,2019),]
...
nz_wcsi_mds_contrast <- ggplot(data = nz_wcsi_full_mds.dt) +
  geom_point(aes(x = NMDS1, y = NMDS2), color = "#AAF400") +
  stat_ellipse(aes(x = NMDS1, y = NMDS2)) +
  facet_wrap(~year)+
  theme_classic()
...
```{r}
scs_summer_years <- unique(FishGlob_clean.noNA[survey_unit == "SCS-SUMMER",year])
#year 2 = 1988,  final year = 2019

scs_summer <- FishGlob_clean.noNA[survey_unit == "SCS-SUMMER" & year %in% c(1988,2019),]
...
scs_summer_mds_contrast <- ggplot(data = scs_summer_full_mds.dt) +
  geom_point(aes(x = NMDS1, y = NMDS2), color = "#DEA0FD") +
  stat_ellipse(aes(x = NMDS1, y = NMDS2)) +
  facet_wrap(~year)+
  theme_classic()
...
name_key <- data.table(survey_unit = c("NZ-WCSI","SEUS-summer","SCS-SUMMER"),`Survey Unit` = c("West Coast South Island, NZ","Southeast US","Scotian Shelf"))

full_mds.dt <- full_mds.dt[name_key, on = "survey_unit"]

mds_contrast <- ggplot(data = full_mds.dt) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = `Survey Unit`), size = 0.5) +
  stat_ellipse(aes(x = NMDS1, y = NMDS2)) +
  facet_wrap(vars(`Survey Unit`, year), nrow = 3, ncol = 2) +
  scale_color_manual(values = c("#AAF400","#DEA0FD","#FCE100")) +
  theme_classic()

mds_contrast

ggsave(mds_contrast, path = here::here("figures"), filename = "mds_contrast_full.jpg",
