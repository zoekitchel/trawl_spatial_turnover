...
all_survey_units_polygons.clean <- ms_erase(all_survey_units_polygons,world[1])
...
map_bray_coef_trend <- ggplot() +
  ...
  scale_fill_manual(values = c("#73BA4D","#E0962C","#cbbfde"),
                    labels = c("Differentiation","Homogenization","No directional change")) +
  ...
  guides(fill = guide_legend(title = "Trend in spatial\ndissimilarity"),
         color = guide_legend(title = "Trend in spatial\ndissimilarity")) +
  theme_bw() +
  base_theme +
...
```
Merge completed map with coefficient vs. characteristic plots
```{r}
library(cowplot)

#load extra plots
mean_temp_vs_coef <- readRDS(here::here("figures","bray_coef_vs_survey_characteristics","mean_temp_vs_coef.rds"))
spp_num_vs_coef <- readRDS(here::here("figures","bray_coef_vs_survey_characteristics","spp_num_vs_coef.rds"))
mid_lat_vs_coef <- readRDS(here::here("figures","bray_coef_vs_survey_characteristics","mid_lat_vs_coef.rds"))

merge_coef_characteristics <- plot_grid(
  spp_num_vs_coef + theme(legend.position = "null", text=element_text(size=8)),
  mid_lat_vs_coef + theme(legend.position = "null", axis.title.y = element_blank(),axis.text.y = element_blank(), text=element_text(size=8)),
  mean_temp_vs_coef + theme(legend.position = "null", axis.title.y = element_blank(),axis.text.y = element_blank(), text=element_text(size=8)),
  ncol = 3, nrow = 1, labels = c("","c.","d."), label_size = 8, hjust = 1,
  rel_widths = c(1.2,1,1)
  )

merge_coef_characteristics_map <- plot_grid(
  map_bray_coef_trend,
  merge_coef_characteristics, 
  ncol = 1, nrow = 2, labels = c("a.","b."), label_size = 8,
  rel_heights = c(2,1)
)

ggsave(merge_coef_characteristics_map, path = here::here("figures"), filename = "merge_coef_characteristics_map.jpg", height = 7, width = 7, unit = "in")
